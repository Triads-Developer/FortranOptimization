...
!       integer, parameter :: max=60
      ...
      integer :: i, j, k, l, iev, mt, it, itlim_flag, k0, nphreid, nr, ic, id0, ip, ielev_m,&
       ird, atsp_hr, atsp_min
      real :: otsp(nevmax,0:itmax1), otsh(0:itmax1), x1sum, x2sum, x3sum, x4sum, xlat, sxlat,&
       cxlat, xlon, sxlon, cxlon, psr, dtpdh, hgtcr, arrtime, ttobs, hms2s, dgkmlogc,&
       usrc(2), dtim, dlat, dlon, ddep, rlatdg1, rlondg1, dts, p1, p2, theta, glat, glon,&
       atsp, atsp_sec, res_test, minimum_weight, depth_min, depth_max
      character(len=8) :: phtest
      character(len=5) :: blank5
      character(len=30) :: blank30
      character(len=100) :: outfil, outfildat0 
      character(len=132) :: msg, dum1, dum2, file_folder
      character(len=160) :: command_line
      logical :: ex, convrg, ptype, stype, op
      ...
      if (verbose_screen) then
         write (msg,'(3a,i3)') 'mlocoutset: opening ', trim(outfil), ' on unit ', io_depth_phase
         call fyi (trim(msg))
      end if
      open (io_depth_phase,file=outfil,status='new')
      
      ! Output file for depth phase name changes
      outfil = trim(outfile)//'.dpnc'
      if (verbose_screen) then
         write (msg,'(3a,i3)') 'mlocoutset: opening ', trim(outfil), ' on unit ', io_dpnc
         call fyi (trim(msg))
      end if
      open (io_dpnc,file=outfil,status='new')
...
!***********************************************************************
      ...
      do while (it .le. nstep .and. .not.convrg) ! Iteration loop
         ...
         do iev = 1,nev ! Loop over events
            ...
            depth_max = max(depthp(iev,it),depth_max)
  ...
  100             continue
            ...
            case (1) ! Convergence tests are done on final event hypocenters
               ...
               do iev = 1, nev
                  ...
                  if (abs(ddep) .gt. cl_dep_c) then
                     convrg = .false.
                     write (io_log,'(i4,a,f10.3,a,f10.3)') iev, ' deo', abs(ddep), ' > ', cl_dep_c
         ...
         if (convrg .or. it .eq. nstep) then
            ...
            if (calibration) then
               ...
               if (cv_perturb) call cluster_vector_perturb (it)
...
!*****************************************************************************************
      ...
      if (gccelout) then
         ...
         call mlocout_gccel (it) ! GCCEL format (~_gccel.dat)
         call gccel_hypocentroid (it) ! Entry for doc.kml
         call gccel_epicenters (it) ! Entry for epicenters.kml
      end if

      if (blocout) then
         call fyi ('mlocset: BayesLoc file')      
         call mlocout_bloc (it) ! BAYESLOC format (.bloc)
      end if
      ...
      if (nitomo .gt. 0) then ! Tomography output files (.TOMO_PHASE.ITOMO.tomo)
         ...
         do i = 1,nitomo
            call mlocout_tomo (it, tomo_phase(i), itomo(i), file_folder)
         end do
      end if
      
      if (ttou) then
         if (.not.calibration .and. .not.direct_cal)&
      ...
      if (gmtp) then
         ...
         if (tt9) call tt_s_minus_p (it) ! S-P times
