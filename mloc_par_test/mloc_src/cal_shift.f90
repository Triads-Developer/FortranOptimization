...
! calculate the shift needed to bring the cluster into best alignment (the
! "optimal" shift vector). The original method (mode = 0) was just to take
! the average of all the individual shift vectors. this is still supported
! in the code but to use it you would need to edit the value of "mode" in
! the call to this subroutine and recompiled. The preferred way  (mode= 1,
! hard-wired in the code) is to take into account the uncertainties of the
! calibration data and the cluster vectors, through their covariance
! matrices, and also to consider the possibility of bias that is not
! included in the formal covariance matrices. Covariance matrices for
! calibration locations are input by the user, using the command "cal_".
! 
! The calibration event covariance matrices (rcv) are assumed to be
! already scaled to a 90% confidence level. This could be done, for
! example, by applying a utility program that converts from the confidence
! ellipse calculated in some other code (e.g., a single event location
! code) to an equivalent covariance matrix. In the case of InSAR analysis
! or geologic information (fault trace) it may be seat-of-the-pants. The
! covariance matrices for cluster vectors (ccv) are first converted to 90%
! confidence ellipses with the normal Bayesian approach, then converted
! back to raw covariancess (subroutine ell2cv), after which they can be
! added to the calibration event covariancess (because they are completely
! independent estimates), yielding the combined covariances (scv). These
! are converted (no scaling) directly to 90% confidence ellipses for the
! combination. The area of these combined confidence ellipses is used for
! inverse weighting of the shift vectors estimated for each calibration
! event, to get the estimate of the optimal shift vector for the entire
! cluster.
! 
! The uncertainty of the optimal shift vector (gcv) is based on the
! weighted (same weights as for the individual shift vectors) combination
! of the combined covariancess (scv), expressed as a confidence ellipse
! (90%). This is only part of the uncertainty, however. There is
...
! azimuthal coverage) has caused bias in the estimate of a cluster vector
! in the hypocentroidal decomposition analysis. Such bias is seen by
! comparing the individual estimates of shift vector to the optimal shift
! vector. If there are departures that are statistically unlikely, given
! the alleged confidence of the calibration locations and cluster vectors,
! then there is a need to account for the unmodeled error. Graphically, it
! is easily seen by plotting residual shift vectors (subtract the optimal
! shift vector) and comparing to the confidence ellipses of each shift
! vector (scv).
! 
! Instead of taking the RMS of the residual shift vectors to estimate the
! level of inconsistency (this is too sensitive to outliers and
! overestimates the uncertainty), I do a test based on the coverage
! statistic for the residual calibration shift vectors, for which the
! cumulative binomial probability distribution gives the probability of
! the observed number of "uncovered" vectors. See 'rdbt_test2' for more
! explanation. If the null hypothesis is rejected, the test is repeated
! after adding a small amount to each covariance matrix, equivalent to
! adding a circular uncertainty. When the null hypothesis cannot be
! rejected, this extra covariance is converted to a "radius of doubt",
! which is added to the modelled uncertainty (gcv) to yield an "augmented
! GT covariance matrix" (agcv). The area of the equivalent ellipse is
! converted to a circular area, and the radius of that circle is a useful
! measure of the calibration level of the cluster. The calibration levels
! of individual events (accv) are found by adding their cluster vector
! covariances to agcv.
! 
! I brought back the older algorithm (rdbt_test1) for radius of doubt
! (v8.1) that is based on a test of the hypothesis that all residual shift
! vectors have zero length. I have kept the test based on coverage
! statistics for now, but it has the problem that we seldom have more than
! a couple calibration events and the 90% coverage requirement translates
! into 100% coverage requirement if there are fewer than 10 calibration
! events. If there are 10 or more calibration events then the larger of
! the two estimates is used. Otherwise the test based on zero-length
! residual vectors is used.
      ...
      if (cv_perturb) then
         write (msg,'(a,f7.3)') 'cal_shift: cluster vectors have been scaled by ',&
          perturbation+1.0, ' before the calibration shift'
         call warnings (trim(msg))
         write (io_cal,'(a)') trim(msg)
      end if
...
!            write (io_cal,'(i3,2f8.2)') iev, rsvx(iev), rsvy(iev)
      ...
      write (io_cal,'(/a,f4.2)') 'Threshold percentage of probability for radius of doubt test: ', pc2
