...
! Read .hed and .tbl files for tau-p calculations.
   ...
   common /tabc/ us(2), pt(jout), tau(4,jout), xlim(2,jout), xbrn(jbrn,3), dbrn(jbrn,2),&
    xn, pn, tn, dn, hn, jndx(jbrn,2), idel(jbrn,3), mbr1, mbr2
       
   double precision :: zs, pk, pu, pux, tauu, xu, px, xt, taut, coef, tauc, xc, tcoef, tp
   real :: odep, fcs
   integer :: nin, nph0, int0, ki, msrc, isrc, nseg, nbrn, ku, km, nafl, indx, kndx, iidx,&
    jidx, kk
   common /brkc/ zs, pk(jseg), pu(jtsm0,2), pux(jxsm,2), tauu(jtsm,2), xu(jxsm,2), px(jbrn,2),&
    xt(jbrn,2), taut(jout), coef(5,jout), tauc(jtsm), xc(jxsm), tcoef(5,jbrna,2), tp(jbrnu,2),&
    odep, fcs(jseg,3), nin, nph0, int0(2), ki, msrc(2), isrc(2), nseg, nbrn, ku(2), km(2),&
    nafl(jseg,3), indx(jseg,2), kndx(jseg,2), iidx(jseg), jidx(jbrn), kk(jseg)
   ...
   if (log_taup) write (io_taup_log,'(a)') 'tabin: opening '//trim(basename)//'.hed'
   ...
   if (log_taup) write (io_taup_log,'(a,i4)') 'tabin: jseg = ', jseg
   ...
   if (log_taup) write (io_taup_log,'(a)') 'tabin: opening '//trim(basename)//'.tbl'
...
9     if (nafl(k,1) .gt. 0 .and. (phcd(i)(1:1) .eq. 'P'.or. phcd(i)(1:1) .eq. 'S')) cycle
      do j = 1,6
...
subroutine depset (dep, usrc)
   ...
   dos = .false.
   do i = 1,nseg
      if (.not.segmsk(i) .or. iidx(i) .gt. 0) cycle
...
!      print *, 'i iidx nph msrc nafl =', i, iidx(i), nph,msrc(nph), nafl(i,1)
      if (iidx(i) .gt. 0 .or. (msrc(nph) .le. 0 .and. nafl(i,1) .gt. 0)) cycle
      iidx(i) = 1
...
!      print *, 'j jidx indx jndx =', j, jidx(j), indx(i,2), jndx(j,2)
   ...
   if (log_taup) then
      write (io_taup_log,*) 'depset: mbr1 mbr2', mbr1, mbr2
      write (io_taup_log,*) 'depset: msrc isrc odep zs us', msrc, isrc, odep, sngl(zs),&
       sngl(us(1)), sngl(us(2))
      write (io_taup_log,'(/10x,i5/(1x,3i5,f12.6))') ki, (i, iidx(i), kk(i), pk(i), i = 1,nseg)
   end if
...
!*****************************************************************************************      
      ...
      logical noend, noext
      double precision tup(jrec), umod, zmod, tauus1(2), tauus2(2), xus1(2), xus2(2), ttau,&
       tx, sgn, umin, dtol, u0, u1, z0, z1, fac, du, eps
      real tol, ztol
!       integer lpower, nph, ks, i, n1, k2, k, k1, ms, mu, is, j, mod, iph, l, lp, kph, i1, i2, m
      integer lpower, nph, ks, i, n1, k2, k, k1, ms, mu, is, j, iph, l, lp, kph, i1, i2, m
      ...
      if (log_taup) write (io_taup_log,*) 'depcor: nph nph0', nph, nph0
      ...
      umin = us(nph)
      ks = isrc(nph)
      if (log_taup) write (io_taup_log,*) 'depcor: ks us', ks, sngl(umin)
      do i = 1,ks
         if (pm(i,nph) .gt. umin) cycle
         umin = pm(i,nph)
      end do
 ...
 4    k2 = i
      if (log_taup) write (io_taup_log,*) 'depcor: k2 umin', k2, sngl(umin)
 ...
 6    noext = .true.
      ...
      ! If it is necessary to interpolate, see if appropriate integrals have already been read in.
 ...
 11   do i=1,ku(nph)
      ...
      if (log_taup) write (io_taup_log,*) 'depcor: bkin', ks, sngl(sgn), sngl(tauu(1,nph)),&
       sngl(xu(1,nph))

      ! Fiddle pointers.
 8    msrc(nph) = ks
      if (log_taup) write (io_taup_log,*) 'depcor: msrc sgn', msrc(nph), sngl(sgn)
      noend = .false.
      ...
      k1 = k2 - 1
      if (noend) k1 = k2
      if (log_taup) write (io_taup_log,*) 'depcor: noend noext k2 k1', noend, noext, k2, k1
      if (noext) go to 14
      ...
      mu = 1
      if (log_taup) write (io_taup_log,*) 'depcor: u0 z0', sngl(u0), sngl(z0)
      if (log_taup) write (io_taup_log,*) 'depcor: u1 z1', sngl(u1), sngl(z1)
      do k = 1,k1
         call tauint (pu(k,nph), u0, u1, z0, z1, ttau, tx)
         tauc(k) = tauu(k,nph) + sgn*ttau
         if (dabs(pu(k,nph)-pux(mu,nph)) .gt. dtol) cycle
         xc(mu) = xu(mu,nph) + sgn*tx
         if (log_taup) write (io_taup_log,*) 'depcor: up x:  k mu', k, mu, sngl(xu(mu,nph)),&
          sngl(xc(mu))
         mu = mu + 1
      end do
      go to 39
 ...
 39   xus1(nph) = 0d0
      ...
      ! This loop may be skipped only for surface focus as range is not available for all ray parameters.
      ...
      tauus1(nph) = tauc(k1)
      xus1(nph) = xc(mu)
      if (log_taup) write (io_taup_log,*) 'depcor: is ks tauus1 xus1', is, ks,&
       sngl(tauus1(nph)), sngl(xus1(nph)), '  *'
      go to 33
 ...
 42   if (dabs(zm(is,nph) - zs) .le. dtol) go to 33
      ! Unless the source is right on a sample slowness, one more partial integral is needed.
 ...
 33   if (pm(is+1,nph) .lt. umin) go to 41
      ! If we are in a high slowness zone, we will also need to integrate down to the turning
      ! point of the shallowest down-going ray.
      u1 = us(nph)
      z1 = zs
      do i = is+1,mt(nph)
         u0 = u1
      ...
      if (log_taup) write (io_taup_log,*) 'depcor: is ks tauus2 xus2', is, ks,&
       sngl(tauus2(nph)), sngl(xus2(nph))

 36   z1 = zmod(umin,i-1,nph)
      if (dabs(z0-z1) .le. dtol) go to 41
      ! Unless the turning point is right on a sample slowness, one more partial integral is needed.
 ...
 41   iph = mod(nph,2) + 1
      ...
      tauus1(iph) = 0d0
      tauus2(iph) = 0d0
      if (nph .eq. 1) go to 59
      if (umin .gt. pu(ku(1)+1,1)) go to 53
      ...
      do j = 1,nbrn
         if ((phcd(j)(1:2) .ne. 'sP' .and. phcd(j)(1:2) .ne. 'SP') .or. px(j,2) .le. 0d0) cycle
         if (log_taup) write (io_taup_log,*) 'depcor: j phcd px umin =', j, ' ', phcd(j),&
          px(j,1), px(j,2), umin
 ...
 59   do j = 1,nbrn
         if ((phcd(j)(1:2) .ne. 'pS' .and. phcd(j)(1:2) .ne. 'PS') .or. px(j,2) .le. 0d0) cycle
         if (log_taup) write (io_taup_log,*) 'depcor: j phcd px umin =', j, ' ', phcd(j),&
          px(j,1), px(j,2), umin
         if (umin .ge. px(j,1) .and. umin .lt. px(j,2)) go to 45
      end do
      ...
      ! Do the integral.
 45   j = 1
      if (log_taup) write (io_taup_log,*) 'depcor: do pS or sP integral - iph =', iph
      do i = 2,mt(iph)
 ...
 47   z1 = zmod(umin,j,iph)
      ...
      tauus1(iph) = tauus1(iph) + ttau
      xus1(iph) = xus1(iph) + tx
      if (log_taup) write (io_taup_log,*) 'depcor: is ks tauusp xusp', j, ks,&
       sngl(tauus1(iph)), sngl(xus1(iph))
...
!     if (odep .ge. deplim .or. odep .le. .1) go to 43
      ...
      ! If the source is very shallow, we will need to insert some extra ray parameter
      ! samples into the up-going branches.
 
 58   du = amin1(1e-5+(odep-.4)*2e-5,1e-5)
      if (log_taup) write (io_taup_log,*) 'depcor: Add:  nph is ka odep du us =', nph, is,&
       ka, odep, sngl(du), sngl(us(nph))
      lp = lpower
      k = 0
      do l = ka,1,-1
         k = k + 1
         ...
         if (log_taup) write (io_taup_log,*) 'depcor: l k ua taua', l, k, sngl(ua(k,nph)),&
          sngl(taua(k,nph))
 54      if (dabs(zm(is,nph)-zs) .le. dtol) cycle
         ! Unless the source is right on a sample slowness, one more partial integral is needed.
 ...
 43   j = 1
      if (log_taup) then
         write (io_taup_log,*) 'depcor: mu', mu
         write (io_taup_log,*) 'depcor: killer loop:'
      end if
      do i = 1,nseg
         if (.not.segmsk(i)) cycle
         if (log_taup) write (io_taup_log,*) 'depcor: i iidx nafl nph', i, iidx(i),&
          nafl(i,1), nph
         if (iidx(i) .gt. 0 .or. iabs(nafl(i,1)) .ne. nph .or.&
          (msrc(nph) .le. 0 .and. nafl(i,1) .gt. 0)) cycle
         ...
         i1 = indx(i,1)
         i2 = indx(i,2)
         if (log_taup) write (io_taup_log,*) 'depcor: i1 i2 sgn iph', i1, i2, sngl(sgn), iph
         m = 1
         do k = i1,i2
    ...
    21      tau(1,k) = taut(k) + sgn*tauc(m)
         ...
         k = i2
         if (log_taup) write (io_taup_log,*) 'depcor: k m', k, m
         go to 24
         
    22   if (log_taup) write (io_taup_log,*) 'depcor: k m', k, m
         ...
         pt(k) = umin
         fac = fcs(i,1)
         if (log_taup) write (io_taup_log,*) 'depcor: ki fac', ki, sngl(fac)
         tau(1,k) = fac*(tauus1(iph)+tauus2(iph)+tauus1(kph)+tauus2(kph))+sgn*tauus1(nph)
         if (log_taup) write (io_taup_log,*) 'depcor: nph iph kph tauus1 tauus2 tau =',&
          nph, iph, kph, sngl(tauus1(1)), sngl(tauus1(2)), sngl(tauus2(1)), sngl(tauus2(2)),&
          sngl(tau(1,k))
    24   m = 1
    ...
    37   if (log_taup) write (io_taup_log,*) 'depcor: j jndx jidx', j, jndx(j,1), jndx(j,2),&
          jidx(j), ' ', phcd(j)
         do l = 1,2
    ...
    27      xbrn(j,l) = xt(j,l) + sgn*xc(m)
            if (log_taup) write (io_taup_log,*) 'depcor: x up:  j l m  ', j, l, m
            cycle
            
    29      xbrn(j,l) = fac*(xus1(iph)+xus2(iph)+xus1(kph)+xus2(kph))+sgn*xus1(nph)
            if (log_taup) then
               write (io_taup_log,*) 'depcor: x up:  j l end', j, l
               write (io_taup_log,*) 'depcor: nph iph kph xus1 xus2 xbrn =', nph, iph, kph,&
                sngl(xus1(1)), sngl(xus1(2)), sngl(xus2(1)), sngl(xus2(2)), sngl(xbrn(j,l))
            end if
         end do
         if (j .ge. nbrn) cycle
         j = j + 1
...
!*****************************************************************************************      
      subroutine bkin (lu, nrec, nlen, buf)
      
      ! Reads a block of nlen double precision words into array buf(len)
      ...
      integer lu, nrec, nlen, i
      double precision buf(nlen), tmp
...
!   tau, and x are all double precision.
 ...
 7    xx = dasin(dmax1(dmin1((b*pti-sqk)/(ptk*dabs(pti-b)),1d0),-1d0))&
       - dasin(dmax1(dmin1((b*ptj-sqk)/(ptk*dabs(ptj-b)),1d0),-1d0))
      x = -ptk*xx/sqb
 ...
 8    xx = dsign(1.5707963267948966d0,b-pti) -&
       dasin(dmax1(dmin1((b*ptj-sqk)/(pti*dabs(ptj-b)),1d0),-1d0))
      x = -pti*xx/sqb
 ...
 12   xx = dasin(dmax1(dmin1((b*pti-sqk)/(ptj*dabs(pti-b)),1d0),-1d0)) -&
       dsign(1.5707963267948966d0,b-ptj)
      x = -ptj*xx/sqb
...
!     logical log
      double precision pmn,dmn,dmx,hm,shm,thm,p0,p1,tau0,tau1,x0,x1,pe,pe0,spe0,scpe0,pe1,&
       spe1,scpe1,dpe,dtau,dbrnch,cn,x180,x360,dtol,ptol,xmin
!       integer i, i1, i2, k, j, jb, is, int, nn, mxcnt, mncnt, ios, mod
      integer i, i1, i2, k, j, jb, is, int, nn, mxcnt, mncnt, ios
      ...
      common /brkc/ zs, pk(jseg), pu(jtsm0,2), pux(jxsm,2), tauu(jtsm,2), xu(jxsm,2),&
       px(jbrn,2), xt(jbrn,2), taut(jout), coef(5,jout), tauc(jtsm), xc(jxsm), tcoef(5,jbrna,2),&
       tp(jbrnu,2), odep, fcs(jseg,3), nin, nph0, int0(2), ki, msrc(2), isrc(2), nseg, nbrn,&
       ku(2), km(2), nafl(jseg,3), indx(jseg,2), kndx(jseg,2), iidx(jseg), jidx(jbrn), kk(jseg)
      double precision zs, pk, pu, pux, tauu, xu, px, xt, taut, coef, tauc, xc, tcoef, tp
      real odep, fcs
      ...
      i1 = jndx(jb,1)
      i2 = jndx(jb,2)
      if (log_taup) write (io_taup_log,*) 'spfit: jb i1 i2 pt =', jb, i1, i2, sngl(pt(i1)),&
       sngl(pt(i2))
 ...
 14   newgrd = .false.
      ...
      if (log_taup) write (io_taup_log,*) 'spfit: px newgrd =', sngl(px(jb,2)), newgrd
      if (.not.newgrd) go to 10
      k = mod(int-1,2) + 1
      if (int .ne. int0(k)) makgrd = .true.
      if (log_taup) write (io_taup_log,*) 'spfit: int k int0 makgrd =', int, k, int0(k), makgrd
      if (int .gt. 2) go to 12
!     call query('Enter xmin:',log)
!     read *,xmin
!     xmin=xmin*xn
      xmin = xn*amin1(amax1(2.*odep,2.),25.)
      if (log_taup) write (io_taup_log,*) 'spfit: xmin =', xmin, xmin/xn
      call pdecu (i1, i2, xbrn(jb,1), xbrn(jb,2), xmin, int, i2)
      jndx(jb,2) = i2
 12   nn = i2 - i1 + 1
      if (makgrd) call tauspl (1, nn, pt(i1), tcoef(1,1,k))
      if (log_taup) write (io_taup_log,301,iostat=ios) jb, k, nn, int, newgrd, makgrd,&
       xbrn(jb,1), xbrn(jb,2), (i,pt(i-1+i1), tau(1,i-1+i1), (tcoef(j,i,k),j=1,5),i=1,nn)
...
!     call appx(i1,i2,xbrn(jb,1),xbrn(jb,2))
      if (log_taup) write (io_taup_log,300) (i, pt(i), (tau(j,i),j=1,3),i=i1,i2)
 300  format (/(1x,i5,4f12.6))
    ...
    4    if (prnt(1)) write (10,100,iostat=ios) disc, j, pt(j), (tau(k,j),k=1,4),&
          (cn*xlim(k,j),k=1,2)
    100  format (1x,a,i5,f10.6,1p,4e10.2,0p,2f7.2)
         dmx = dmax1(dmx,xlim(2,j))
         j = i
      end do
      if (log_taup) write (io_taup_log,100,iostat=ios) '   ', j, pt(j)
      xbrn(jb,1) = dmn
      xbrn(jb,2) = dmx
      xbrn(jb,3) = pmn
      idel(jb,1) = 1
      ...
      do j = 1,nbrn
         i = mod(i,nbrn) + 1
         if (phcd(i)(1:1) .eq. phcd(jb) .and. phcd(i)(2:2) .ne. 'P' .and. (pe .ge. px(i,1)&
          .and. pe .le. px(i,2))) go to 9
      end do
      go to 1
      
 9    phcd(jb) = phcd(i)
      if (dabs(pt(i2)-pt(jndx(i,1))) .le. dtol) phcd(jb) = phcd(i-1)
 1    if (log_taup) write (io_taup_log,102)
 102  format ()
      if (dbrn(jb,1) .le. 0d0) go to 3
      dbrn(jb,1) = dmx
      dbrn(jb,2) = dbrnch
      if (log_taup) write (io_taup_log,101,iostat=ios) phcd(jb), (jndx(jb,k),k=1,2),&
       (cn*xbrn(jb,k),k=1,2), xbrn(jb,3), (cn*dbrn(jb,k),k=1,2), (idel(jb,k),k=1,3), int,&
       newgrd, makgrd
 101  format (1x,a,2i5,2f8.2,f8.4,2f8.2,4i3,2l2)
      go to 15
      
 3    if (log_taup) write (io_taup_log,103,iostat=ios) phcd(jb), (jndx(jb,k),k=1,2),&
       (cn*xbrn(jb,k),k=1,2), xbrn(jb,3), (idel(jb,k),k=1,3), int, newgrd, makgrd
...
!*****************************************************************************************      
      ...
      if (log_taup) write (io_taup_log,*) 'pdecu: ua =', sngl(ua(1,int))
      if (ua(1,int) .le. 0d0) go to 17
      if (log_taup) write (io_taup_log,*) 'pdecu: fill in new grid'
      k = i1 + 1
      do i =1,ka
         pt(k) = ua(i,int)
         tau(1,k) = taua(i,int)
...
14       if (dabs(x-xs) .le. xmin) go to 15
 ...
 9    k = i1
      do i = is,i2
         if (pt(i) .lt. 0d0) cycle
         k = k + 1
         pt(k) = pt(i)
         tau(1,k) = tau(1,i)
      end do
 19   len = k
      if (log_taup) write (io_taup_log,300) (i, pt(i), tau(1,i),i=i1,len)
300   format (/(1x,i5,0p,f12.6,1p,d15.4))

      return
      
      end subroutine pdecu
...
!   Rewritten on 23 December 1983 by R. Buland.
      ...
      double precision del(5), sdel(5), deli(5), d3h(4), d1h(4), dih(4), d(4), ali, alr,&
       b3h, b1h, bih, th0p, th2p, th3p, th2m
...
!          end do
      ...
      ! seperately because of the singularities in the second derivitive at p;n.
 10   do j = 1,4
         m = 1
         ! Update temporary variables for G;i-1.
!          do k = 2,5
...
!          end do
    ...
    6    alr = (sdel(2)+deli(2)*del(3)-2d0*sdel(3))/(d1h(1)+dih(1)*del(3))
         d(2) = 1d0/(.125d0*del(2)*sdel(2)-(.75d0*sdel(2)+.375d0*deli(2)*del(3)-sdel(3))*del(3)-(.125d0*d3h(1)-(.75d0*d1h(1)+&
          .375d0*dih(1)*del(3))*del(3))*alr)
         d(1) = alr*d(2)
         if (j-2 .lt. 0) then ! No additional constraints are required for G;n-2.
            d(3) = -((d3h(2)-d1h(2)*del(4))*d(2)+(d3h(1)-d1h(1)*del(4))*d(1))/(d3h(3)-d1h(3)*del(4))
            d(4) = (d3h(3)*d(3)+d3h(2)*d(2)+d3h(1)*d(1))/(del(4)*sdel(4))
         else if (j-2 .eq. 0) then ! For G;n-1 constrain G;n-1(p;n) to be .25.
            d(3) = (2d0+d3h(2)*d(2)+d3h(1)*d(1))/(del(3)*sdel(3))
         end if
         
         ! Construct the contributions G;i-1(p;i-2) and G;i-1(p;i).
    9    if (j .le. 2) coef(1,l) = (.125d0*del(3)*sdel(3)-(.75d0*sdel(3)+.375d0*deli(3)*del(4)-sdel(4))*del(4))*d(3)-&
          (.125d0*d3h(2)-(.75d0*d1h(2)+.375d0*dih(2)*del(4))*del(4))*d(2)-(.125d0*d3h(1)-(.75d0*d1h(1)+.375d0*dih(1)*del(4))*&
          del(4))*d(1)
...
!*****************************************************************************************      
 ...
 9    if (dabs(x(1)-pi) .gt. dtol) go to 7
      ...
      x(2) = -10d0
 7    do j = mbr1,mbr2
         if (jndx(j,2) .gt. 0) call findtt (j, x, max, n, tmp, tmp(1,2), tmp(1,3), tmp(1,4), ctmp)
      end do
      ...
      do i = 1,n
         j = iptr(i)
         if (k .le. 0) go to 8
         if (phnm(k) .eq. ctmp(j) .and. abs(tt(k)-tmp(j,1)) .le. atol) cycle
    8    k = k + 1
         tt(k) = tmp(j,1)
         dtdd(k) = tmp(j,2)
         dtdh(k) = tmp(j,3)
...
!*****************************************************************************************      
      ...
      do ij = idel(jb,1),idel(jb,2)
       ...
       2    do jj = 1,2
               if (jj .eq. 2) go to 6
               arg = 9d0*tau(4,j)*tau(4,j) + 32d0*tau(3,j)*(x-tau(2,j))
          ...
          6    dps = (tau(2,j)-x)/(2d0*tau(3,j)*dps)
               dp = dsign(dps*dps,dps)
          7    if (dp .lt. p1-delp .or. dp .gt. p0+delp) cycle
               if (n .ge. max) go to 13
               n = n + 1
               ps = pt(ie) - dp
    ...
    12   if (x .lt. dbrn(jb,1) .or. x .gt. dbrn(jb,2)) cycle
         if (n .ge. max) go to 13
         j = jndx(jb,1)
         i = jndx(jb,2)
         dp = pt(i) - pt(j)
...
!   December 1979 by R. Buland.
      ...
      ! If we fell off the end of the string, branch if there were any non-blank characters.
 5    if (k .gt. 0) go to 6
      ...
      ! Handle a string with no question mark or colon but at least one non-blank character.
 6    i = k
 ...
 102  format(a4)
      ...
      ! If the response is no set log to false.  Otherwise repeat the question.
...
!   on 11 December 1985 by R. Buland.
      ...
      do i = 1,n
         if (ifl .lt. 0) go to 2
         if (ia(i:i) .eq. '''') nfl = -nfl ! Look for single quotes.
         if (nfl .lt. 0) cycle ! If we are in a quoted string skip the conversion.
         ! Do the conversion.
    2    ib = ichar(ia(i:i))
         if (ib .lt. 65 .or. ib .gt. 90) cycle
         ia(i:i) = char(ib+32)
      end do
 
      return
      
      end subroutine uctolc
...
!   27 September 1983 by R. Buland.
      ...
      integer :: i, j, k, l, m, n, ij, it, kk, ib
      integer :: iptr(n), il(10), iu(10)
...
!*****************************************************************************************      
      ...
      common /brkc/ zs, pk(jseg), pu(jtsm0,2), pux(jxsm,2), tauu(jtsm,2), xu(jxsm,2),&
       px(jbrn,2), xt(jbrn,2), taut(jout), coef(5,jout), tauc(jtsm), xc(jxsm), tcoef(5,jbrna,2),&
       tp(jbrnu,2), odep, fcs(jseg,3), nin, nph0, int0(2), ki, msrc(2), isrc(2), nseg,&
       nbrn, ku(2), km(2), nafl(jseg,3), indx(jseg,2), kndx(jseg,2), iidx(jseg), jidx(jbrn),&
       kk(jseg)
      double precision zs, pk, pu, pux, tauu, xu, px, xt, taut, coef, tauc, xc, tcoef, tp
      real odep, fcs
      integer nin, nph0, int0, ki, msrc, isrc, nseg, nbrn, ku, km, nafl, indx, kndx, iidx,&
...
!        print *,'jp phcd xbrn',jp,'  ',phcd(jp),xbrn(jp,1)
         if ((phcd(jp) .eq. 'Pg' .or. phcd(jp) .eq. 'Pb' .or. phcd(jp) .eq. 'Pn' .or.&
          phcd(jp) .eq. 'P') .and. xbrn(jp,1) .le. 0d0) go to 3
      end do
      jp = 0
 ...
 3    do js = mbr1,mbr2
   !     print *,'js phcd xbrn',js,'  ',phcd(js),xbrn(js,1)
         if ((phcd(js) .eq. 'Sg' .or. phcd(js) .eq. 'Sb' .or. phcd(js) .eq. 'Sn' .or.&
          phcd(js) .eq. 'S') .and. xbrn(js,1) .le. 0d0) go to 1
      end do
      js=0
...
!   query mode) are also available.
      ...
      common /brkc/ zs, pk(jseg), pu(jtsm0,2), pux(jxsm,2), tauu(jtsm,2), xu(jxsm,2),&
       px(jbrn,2), xt(jbrn,2), taut(jout), coef(5,jout), tauc(jtsm), xc(jxsm),&
       tcoef(5,jbrna,2), tp(jbrnu,2), odep, fcs(jseg,3), nin, nph0, int0(2), ki, msrc(2),&
       isrc(2), nseg, nbrn, ku(2), km(2), nafl(jseg,3), indx(jseg,2), kndx(jseg,2),&
       iidx(jseg), jidx(jbrn), kk(jseg)
      double precision zs, pk, pu, pux, tauu, xu, px, xt, taut, coef, tauc, xc, tcoef, tp
      real odep, fcs
      integer nin, nph0, int0, ki, msrc, isrc, nseg, nbrn, ku, km, nafl, indx, kndx, iidx,&
      jidx, kk
...
!   doesn't require a keyword.
      ...
      data cmdlst/'P','PKiKP','PcP','pP','pPKiKP','sP','sPKiKP','ScP','SKP','PKKP',&
       'SKKP','PP','S','ScS','sS','pS'/
 ...
 1    all = .false.
    ...
    9    j = j + 1
       ...
       5    if (l .ge. 8) cycle
            if (phtmp(l:l+1) .ne. 'ab' .and. phtmp(l:l+1) .ne. 'ac' .and. phtmp(l:l+1) .ne. 'df') cycle
            phtmp(l:) = ' '
            go to 4
            
         end do
   !     print *,'j phcd phtmp =',j,' ',phcd(j),' ',phtmp
