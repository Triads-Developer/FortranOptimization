...
! File of stations that are suspected off reporting bogus depth phases
   ...
   if (command_help) then
      write (*,'(13(a/))')&
       ...
       'the ".depth_phases" file.',&
       '   The generic station file format (type 3) is used for this list because it carries',&
       'fields for epoch dates "on" and "off". When used for bogus depth phase reporting, however,',&
       'the dates are interpreted as the epoch during which bogus depth phases have been reported.',&
       'These dates are optional, and other station information that can be provided in this format',&
       'is not used by MLOC. It can be useful for station identification, however. The only required',&
       'field is station code in columns 1-5, but the first line must have "3" in the first column',&
...
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       ...
       'i.e., travel time model inadequacies, this is usually negligible.'
...
! Export mloc calibrated hypocenters for use as priors in BayesLoc
! by Ezgi Karasözen 3/9/2016
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       '".bloc") will be written. The format is designed for easy import of mloc calibrated',&
       'hypocenters into BayesLoc as priors. Default is "off". Issuing the command with no',&
       'argument toggles the current state, or the arguments "on" or "off" can be used to',&
...
! Calibration event
   ...
   if (command_help) then
      write (*,'(16(a/))')&
       ...
       'semi-minor axis length, semi-major axis length (both in km). The command can be given',&
       'multiple times for an event, in which case the last command will define the calibration',&
       'parameters.'
      return
   end if
   ...
   if (nvar .eq. 11) then
      ...
      if (.not.cal_event(iev,1)) then ! First calibration command for this event
         calibration = .true.
         cal_event(iev,1) = .true.
         ncal(1) = ncal(1) + 1
         write (msg,'(a,i3,1x,2a)') 'proc_cal_: event ', iev, trim(evtnam(iev)), ' is declared as a calibration event'
         call fyi (trim(msg))
      else
         write (msg,'(a,i3,1x,2a)') 'proc_cal_: event ', iev,  trim(evtnam(iev)), ' has multiple calibration commands'
         call fyi (trim(msg))
      end if
      ot_cal = cal_par(iev) .eq. 'h' ! Origin time for the cluster can be calibrated
      ...
      if (verbose_screen) then
         write (msg,'(a,3f8.3,2i3,f6.2,6f8.2)') 'proc_cal_: ', cal_lat(iev,1), cal_lon(iev,1), cal_dep(iev,1), cal_hr(iev,1),&
...
!***********************************************************************************************************************************
subroutine proc_gcat (command_help)

! Global Catalog of Calibrated Earthquake Locations (GCCEL) output file.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       '"gcat" (Global CATalog) specifies that a ~_gccel.dat file will be written.',&
       'This is a single file similar to the _datf.mnf file, but with additional information',&
       'used in the GCCEL project. The command takes a single optional',&
       ...
       'is good). The commentary will be added to the ~_gccel.dat file as comment records.'
      return
   end if
   ...
   if (params(1:1) .ne. ' ') then
      ...
      call warnings ('proc_gcat: no commentary file has been specified')
      commentary_fname = 'none'
   end if
   
   ! Create a subdirectory for GCCEL files
   gcat_folder = trim(datadir)//dirsym//trim(basename)//'_gccel'
   command_line = 'mkdir '//trim(gcat_folder)
   call system (trim(command_line))

   if (verbose_screen) call fyi ('proc_gcat: will create a GCCEL file using the commentary file '//trim(commentary_fname))

   return
   
end subroutine proc_gcat
...
! Station elevation correction
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       ...
       'reference plane is the reference ellipsoid. If not, the reference plane is, roughly,',&
       ' the surface (average elevation) of the source region.'
...
!*****************************************************************************************
subroutine proc_cvid (command_help)

! Cluster vector inflation and deflation.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
	   '"cvid" (Cluster Vector Inflation Deflation) Experimental and very ad hoc! When doing',&
	   'indirect calibration of a cluster covering an unusually large region, having greatly',&
	   'separated calibration events, there may be misfit suggesting the entire cluster needs to ',&
	   'be expanded or contracted slightly. This is presumably due to inadequacy of the theoretical',&
	   'TT model. This command takes a perturbation factor that changes the length of all',&
	   'cluster vectors by the same fraction. In the only test done so far an expansion of',&
	   'about 1.7% was needed. The perturbation can be positive or negative. The value is input',&
	   'as a decimal (e.g. "0.017" for 1.7%)'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      if (nvar .eq. 1) then
         ...
         cv_perturb = .true.
         if (verbose_screen) then
            write (msg,'(a,f6.2)') 'proc_cvid: ', perturbation
            call fyi (trim(msg))
         end if
         if (abs(perturbation) .gt. 0.03)  call warnings ('proc_cvid: this is an unusually large perturbation factor!')
      else
         call warnings ('proc_cvid: "cvid" requires one argument, the perturbation factor')
      end if 
   end if
...
!***********************************************************************************************************************************
subroutine proc_damp (command_help)

! Damping for cluster vector changes, subtracts average for each parameter.
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       '"damp" (damping) determines if damping will be applied to the cluster vector changes',&
       'at each iteration. Damping is done by calculating the mean of changes in a parameter',&
       'for all events and subtracting it from the change for each event. This sometimes helps',&
       'convergence. The default is off. The state can be toggled by using the command without',&
       'an argument, or set explicitly with arguments "on" or "off".'
      write (*,'(a,l1/)') 'Current value: ', damping
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call warnings ('proc_damp: illegal argument')
   end if

   if (verbose_screen) then
      if (damping) then
         call fyi ('proc_damp: damping is on')
      else
         call fyi ('proc_damp: damping is off')
      end if
   end if

return

end subroutine proc_damp
...
! Plot a digital elevation model in GMT, using ETOPO1 <http://www.ngdc.noaa.gov/mgg/global/global.html>.
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       ...
       'added to the GMT script, using the ETOPO1 1-minute gridded topography and bathymetry.',&
       'Can be toggled by using the command without an argument, or set explicitly with',&
       'arguments "on" or "off". Default is "off".'
      return
   end if
   
   if (params(1:1) .eq. ' ') then
      plot_dem1 = .not.plot_dem1
   else if (params(1:2) .eq. 'on') then
      plot_dem1 = .true.
   else if (params(1:3) .eq. 'off') then
      plot_dem1 = .false.
   else if (params(1:6) .eq. 'etopo1') then
      call fyi ('proc_dem1: "etopo1" argument is deprecated. ETOPO1 is now the only option. Use "dem1 on"')
      plot_dem1 = .true.
   else if (params(1:4) .eq. 'gina') then
      call fyi ('proc_dem1: "gina" argument is no longer supported. ETOPO1 is now the only option. Use "dem1 on"')
      plot_dem1 = .true.
   else if (params(1:5) .eq. 'globe') then
      call fyi ('proc_dem1: "globe" argument is no longer supported. ETOPO1 is now the only option. Use "dem1 on"')
      plot_dem1 = .true.
   else
      call warnings ('proc_dem1: illegal argument. No topography will be plotted.')
      plot_dem1 = .false.
   end if
...
! (shallower). These values are not used in the relocation, but they are passed through in the .hdf
! file for future reference.
   ...
   if (iev .eq. 0) then ! Cluster default depth, applied to any event that does not have depth explicitly set
      depth_default = value(1)
      if (nvar .gt. 1) call warnings ('proc_dep: cluster default depth does not take uncertainties')
      if (comd(4:4) .ne. 'c') then
         call warnings ('proc_dep: events using the cluster default depth will be given the depth constraint code "c"')
      end if
...
! Fault map for GMT scripts.
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       '"/tables/faults". The command can be issued multiple times.'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      if (n_fault_map .lt. n_fault_map_nmax) then
         ...
         if (ex) then
            ...
            if (verbose_screen) call fyi ('   ...with plot options: '//fault_plot_style)
         else
...
!***********************************************************************************************************************************
subroutine proc_gmtp (command_help)

! Control of plotting with GMT.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       '"gmtp" (GMT Plotting) enables/disables plotting (with GMT). This command is',&
       'provided for unusual cases where mloc must be run on a system with no installation',&
       'of GMT or where the GMT installation is not supported (GMT4 or earlier). The ability',&
       'to monitor the performance of mloc is greatly handicapped if plotting is disabled.',&
       'Default is ".true." (plotting enabled). The logical state is toggled by issuing the',&
       'command with no argument, or set explicitly with "on" or "off" as the argument.'
      write (*,'(a,l1/)') 'Current value: ', gmtp
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call warnings ('proc_gmtp: illegal argument')
   end if

   if (verbose_screen) then
      if (gmtp) then
         call fyi ('proc_gmtp: plotting enabled')
      else
         call fyi ('proc_gmtp: plotting disabled')
      end if
   end if
   
   return

end subroutine proc_gmtp
...
! Command summaries and full help
   ...
   write (*,'(t3,a)') 'cvid: cluster vector inflation/deflation'
   ...
   write (*,'(t3,a)') 'tlog: logging tau-p calculations'
   ...
   write (*,'(t3,a)') 'damp: damping cluster vector changes'
   ...
   write (*,'(t3,a)') 'noxf: ignore x flags'
   ...
   write (*,'(t3,a)') 'gcat: GCCEL output file'
   ...
   write (*,'(t3,a)') 'plog: output of ~.plog file'
   ...
   write (*,'(t3,a)') 'gmtp: Enable/disable GMT plotting'
   write (*,'(t3,a)') 'plot: plotting of selected events'
   write (*,'(t3,a)') 'phrp: plotting of residuals for a specified phase'
   write (*,'(t3,a)') 'pltt: travel time plots'
   write (*,'(t3,a)') 'rdpp: relative depth phase plots for individual events'
   write (*,'(t3,a)') 'rpth: plot direct calibration raypaths'
   ...
   write (*,'(t3,a)') 'cvtt: cluster vector travel time error for differential time data'
   ...
   write (*,'(t3,a)') 'ifsm: search ISC-FDSN station metadata for missing station codes'
   write (*,'(t3,a)') 'nsmd: search NEIC station metadata for missing station codes'
   write (*,'(t3,a)') 'radf: read agency and deployment fields for a given station to resolve station code conflicts'
   write (*,'(t3,a)') 'skip: skip readings from a given station (also by phase and author)'
   write (*,'(t3,a)') 'sstn: read a supplemental station file'
   ...
   write (*,'(t3,a)') 'p_tt: P_ travel time calculation'
   write (*,'(t3,a)') 'rgtt: Rg travel time calculation'
   write (*,'(t3,a)') 'secv: station elevation correction velocities'
   write (*,'(t3,a)') 'taup: Global TT model, using Tau-P formulation'
   write (*,'(t3,a)') 'terr: Timing error correction at a station'
...
! Epicentral distance limits for hypocentroid
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       ...
       'readings at around the Pg/P crossover, in the Pdiff range, and near the caustic for PKP',&
...
!***********************************************************************************************************************************
subroutine proc_ifsm (command_help)

! Check the combined ISC-FDSN station metadata file for missing station codes.
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   character(len=132) :: ifsmfile, msg
   logical :: command_help, ex

   if (command_help) then
      write (*,'(12(a/))')&
       '"ifsm" (ISC-FDSN Station Metadata) determines if the combined ISC-FDSN station metadata (IFSM) file will',&
       ...
       'in the IFSM will NOT be used in the current run; a supplemental',&
       'station file must be referenced in subsequent runs. A file of the matched stations',&
       'is created in the "generic" format (isstn=3). This command only works if the ~.mnf',&
       'files carry a deployment code. Matching is based on station code, deployment',&
       'and date, but the operational date fields (date on and date off) in the current',&
       'version of the ISC-FDSN Station Metadata file are blank. The default epoch',&
       'is 1900-2099 so there is no test on operational epoch at this time. Default is no',&
       'search. Logical state is toggled by issuing the command without an argument, or',&
       'set explicitly with "on" or "off" as the argument.'
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call warnings ('proc_ifsm: illegal argument')
   end if

   if (verbose_screen) then
      if (ifsm) then
         call fyi ('proc_ifsm: ISC-FDSN station metadata will be searched for missing station codes')
      else
         call fyi ('proc_ifsm: ISC-FDSN station metadata will not be searched for missing station codes')
      end if
   end if
   
   ! Check that the ISC-FDSN station metadata file is present
   if (ifsm) then
      ifsmfile = trim(station_path)//dirsym//'ISC-FDSN_stn.dat'
      inquire (file=ifsmfile,exist=ex)
      if (.not.ex) then
         write (msg, '(3a)') 'proc_ifsm: ISC-FDSN station metadata file (', trim(ifsmfile), ') does not exist'
         call warnings (trim(msg))
         ifsm = .false.
      end if
   end if
...
! Output of file with large cluster residuals
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       ...
       'by the single argument. Default is "on" with threshhold = 3.0'
      return
   end if
...
!***********************************************************************************************************************************
subroutine proc_noxf (command_help)
...
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   if (command_help) then
      write (*,'(7(a/))')&
       '"noxf" (NO X Flags) determines if readings flagged with "x" in the event data files',&
       'will be used in the relocation. Normal usage is to honor these flags but on rare',&
       'occasions it may be useful to process readings that have been previously flagged as',&
       'outliers.'
      write (*,'(a,l1/)') 'Current value: ', no_xflags
      return
   end if

   if (params(1:1) .eq. ' ') then
      no_xflags = .not.no_xflags
   else if (params(1:2) .eq. 'on') then
      no_xflags = .true.
   else if (params(1:3) .eq. 'off') then
      no_xflags = .false.
   else
      call warnings ('proc_noxf: illegal argument')
   end if

   if (verbose_screen) then
      if (no_xflags) then
         call fyi ('proc_noxf: readings flagged "x" will be used')
      else
         call fyi ('proc_noxf: readings flagged "x" will not be used (normal)')
      end if
   end if

return

end subroutine proc_noxf
...
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   if (command_help) then
      write (*,'(9(a/))')&
       ...
       'station file must be referenced in subsequent runs. A file of the matched stations',&
       'is created in the "NEIC" format (isstn=5). This command only works if the ~.mnf',&
       'files carry a deployment code. Matching is based only on station code, deployment',&
       'and date. Default is no search. Logical state is toggled by issuing the command',&
       'without an argument, or set explicitly with "on" or "off" as the argument.'
      write (*,'(a,l1/)') 'Current value: ', nsmd
      return
   end if
...
!***********************************************************************************************************************************
subroutine proc_p_tt (command_help)
...
! P_ travel time calculations.
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       '"p_tt" (P_ Travel Time) specifies a custom travel-time model for the P_ ("P bar")',&
       'phase. The model is linear in epicentral distance. It takes three arguments: a',&
       'zero-intercept (sec), slope (sec/degree) and minimum epicentral distance (degrees)',&
       'from which to calculate travel times for P_. Default values are 0., 19.0 and 2.5.'
      return
   end if
   ...
   if (nvar .eq. 3) then
      ...
      p__min = value(3)
      if (verbose_screen) then
         write (msg,'(a,3f8.2)') 'proc_p_tt:', p__a, p__b, p__min
         call fyi (trim(msg))
      end if
   else
      call warnings ('proc_p_tt: "p_tt" requires three arguments')
   end if

   return
   
end subroutine proc_p_tt
...
! Perturb the starting location of all events
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       ...
       'longitude perturbations are in decimal degrees.'
      return
   end if
   ...
   if (nvar .eq. 4) then
      ...
      htimeshift = value(4)
      if (verbose_screen) then
         write (msg,'(a,4f8.3)') 'proc_pert: All hypocenters perturbed by ', hlatshift, hlonshift, hdepthshift, htimeshift
         call fyi (trim(msg))
      end if
   else
...
!*****************************************************************************************
subroutine proc_phrp (command_help)

! Makes a plot of residuals for a single phase.
! May be issued multiple times. Plots are put into a subdirectory.
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       '"phrp" (PHase Residual Plot) makes a plot of the residuals for a specified phase.',&
       'The command takes three arguments: phase name and minimum and maximum epicentral',&
       'distance to plot.'
      return
   end if
   ...
   if (params(1:1) .ne. ' ') then
	  phrp = .true.
	  n_phrp = n_phrp +1
	  if (n_phrp .gt. n_phrp_max) then
	     write (msg,'()') 'proc_phrp: maximum number of plots (', n_phrp_max,') has been reached'
	     call warnings (trim(msg))
	     n_phrp = n_phrp - 1
	     return
	  end if
	  ...
	  ! Find the phase name
	  do j = 1,len(params)
		 if (params(j:j) .eq. ' ') then
			phrp_phase(n_phrp) = params(1:j-1)
			exit
		 end if
	  end do
	  if (lenb(phrp_phase(n_phrp)) .gt. 8) then
	     call warnings ('proc_phrp: illegal phase name: '//trim(phrp_phase(n_phrp)))
	     if (n_phrp .eq. 1) phrp = .false.
	     n_phrp = n_phrp - 1
	     return
	  end if
   
      ! Epicentral distance range
      call decode (params(j+1:len(params)), value, nvar)
      if (nvar .eq. 2) then
		 phrp_delt(n_phrp,1) = value(1)
		 phrp_delt(n_phrp,2) = value(2)
	  else
         call warnings ('proc_phrp: "phrp" requires two arguments for epicentral distance range')
	     if (n_phrp .eq. 1) phrp = .false.
	     n_phrp = n_phrp - 1
	     return
      end if
      ...
      if (verbose_screen) then
		 write (msg,'(a,i2,1x,a,2f6.1)') 'proc_phrp: ', n_phrp, phrp_phase(n_phrp), (phrp_delt(n_phrp,i),i=1,2)
		 call fyi (trim(msg))
	  end if
      
   else
      call warnings ('proc_phrp: "phrp" requires three arguments')
   end if

   return
   
end subroutine proc_phrp
...
! It also keeps S-P readings if direct calibration is being used.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       '30 and 90 degrees, which provides a consistent (albeit biased) estimate of the',&
...
!***********************************************************************************************************************************
subroutine proc_plog (command_help)
...
! plog output file
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       '"plog" (Phase LOG) specifies that a ~.plog file will be written. This is a single',&
       'file with detailed information about changes in phase identification that occurred',&
       'in the current run. It is quite opaque and almost never consulted except during',&
       'code development. Arguments "on" and "off" can be used to set the logical state',&
       'explicitly. Issuing the command without an argument toggles the logical state.',&
       'Default is "off".'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call warnings ('proc_plog: illegal argument')
   end if

   if (plogout) then
      if (verbose_screen) call fyi ('proc_plog: will create a ~.plog file')
   else
      if (verbose_screen) call fyi ('proc_plog: will not create a ~.plog file')
   end if

   return
   
end subroutine proc_plog
...
! Selected events for plotting
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'an integer which specifies which selected-event plots this event will be included in. Limit',&
...
! Read agency and deployment fields from station files and event data files for a specified station.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       'in event data files (.mnf files) and station lists for the specified station code',&
       'will be read and used to resolve station code conflicts. This is only used when a dataset',&
       'contains readings from two or more different stations with the same code. The command',&
       'takes a single argument, a station code. The command can be issued multiple times,',&
       'up to the limit specified by "n_radf_max" (currently 10).'
      return
   end if
   
   if (params(1:1) .ne. ' ') then
      n_radf = n_radf + 1
      if (n_radf .le. n_radf_max) then
         radf_stn(n_radf) = trim(params)
         read_ad = .true.
      else
         call warnings ('proc_radf: maximum number of instances reached')
         n_radf = n_radf - 1
      end if
   else
      call warnings ('proc_radf: "radf" requires a single argument')
   end if

   if (verbose_screen) call fyi ('proc_radf: agency and deployment fields will be read for station '//trim(radf_stn(n_radf)))
...
! times. Plots are put into a subdirectory.
   ...
   if (command_help) then
      write (*,'(5(a/))')&
       '"rdpp" (relative depth phase plot) makes a plots of misfit vs. depth for relative',&
       'depth phases (pP-P, sP-P, pwP). The command takes a single argument. To make a plot',&
       'for a specific event the argument is the event name (as given in the "even" command).',&
       'This command can be issued multiple times, up to the maximum number of events. The',&
       'argument "all" can be used to make plots of all events for which depth phases are reported.'
      return
   end if
   ...
   if (params(1:1) .ne. ' ') then
      if (len_trim(params) .le. 30) then
         rdpp = .true.
         if (trim(params) .eq. 'all') then
            rdpp_all = .true.
            if (verbose_screen) call fyi ('proc_rdpp: relative depth phase plots will be done for all events')
         else
            n_rdpp = n_rdpp + 1
      ...
      call warnings ('proc_rdpp: "rdpp" requires one argument, an event name, or the argument "all"')
   end if
...
! Review of currently-selected parameters and options
   ...
   if (bdp_list) write (*,'(t3,a,t50,2a)') '(bdps) bogus depth phase stations', ': ', trim(bdp_filnam)
   write (*,'(t3,a,t50,a,l1)') '(bias) hypocentroid bias-correction', ': ', bias_corr
   write (*,'(t3,a,t50,a,l1)') '(bloc) BayesLoc file output', ': ', blocout
   write (*,'(t3,a,t50,a,l1)') '(bptc) bounce point topography correction', ': ', bptc
   write (*,'(t3,a,t50,a,3(2f6.1,2x))') '(clim) cluster distance limits', ': ', (clim(i,1),clim(i,2),i=1,nclim)
   select case (tt_corr)
      case (0)
         write (*,'(t3,a,t50,a)') '(corr) station corrections', ': none'
      case (1)
         write (*,'(t3,a,t50,a)') '(corr) station corrections', ': elevation corrections'
   end select
   if (calibration) then
      select case (icaltype)
         case (1)
            write (*,'(t3,a,t50,a)') '(ctyp) indirect calibration type', ': traditional'
         case (2)
            write (*,'(t3,a,t50,a)') '(ctyp) indirect calibration type', ': systematic'
         case (3)
            write (*,'(t3,a,t50,a)') '(ctyp) indirect calibration type', ': optimal'
      end select
   end if
   write (*,'(t3,a,t50,a,f6.2)') '(cvff) radius', ': ', radius_cvff
   if (cv_perturb) write (*,'(t3,a,t50,a,f6.3)') '(cvid) cluster vector inflation/deflation:', ': ', perturbation   
   write (*,'(t3,a,t50,a,l1)') '(damp) damping cluster vector changes', ': ', damping
   write (*,'(t3,a,t50,a,l1)') '(datf) DATF file output', ': ', datfout
   write (*,'(t3,a,t50,2a)') '(dcal) direct calibration', ': ', dcal_pr
   if (plot_dem1) write (*,'(t3,a,t50,a)') '(dem1) topography/bathymetry', ': ETOPO1 DEM'
   if (plot_dem2) write (*,'(t3,a,t50,2a)') '(dem2) high-rez topography', ': ', trim(dem2_filename)
   if (diffdat) write (*,'(t3,a,t50,2a)') '(diff) differential times data', ': ', trim(diffdatfilnam)
   if (epa_plot) then
      do i = 1,n_epa_plot
         write (*,'(t3,a,t50,2a,f5.1,1x,l1)') '(epap) emirical path anomaly plot', ': ',&
          epa_plot_phase(i), epa_plot_distance(i), epa_plot_raypath(i)
      end do
   end if
   write (*,'(t3,a,t50,a,l1)') '(eplt) confidence ellipse plot', ': ', eplt
   write (*,'(t3,a,t50,a,l1)') '(fdhp) focal depth histogram', ': ', fdhp
   write (*,'(t3,a,t50,2a)') '(flag) use data flags', ': ', dflag_pr
   if (fault_map) then
      do i = 1,n_fault_map
         write (*,'(t3,a,t50,2a)') '(fmap) fault map file', ': ', trim(fault_map_filename(i))
      end do
   end if
   write (*,'(t3,a,t50,8a)') '(freh) hypocentroid lat/lon/depth/OT', ': ', fixprh(1), '/', fixprh(2), '/', fixprh(3),&
    '/', fixprh(4)
   write (*,'(t3,a,t50,a,l1)') '(gcat) GCCEL file output', ': ', gccelout
   write (*,'(t3,a,t50,a,l1)') '(gmtp) Enable/disable plotting', ': ', gmtp  
   write (*,'(t3,a,t50,a,3(2f6.1,2x))') '(hlim) hypocentroid limits', ': ', (hlim(i,1),hlim(i,2),i=1,nhlim)
   write (*,'(t3,a,t50,a,l1)') '(ifsm) ISC-FDSN station metadata search', ': ', ifsm
   write (*,'(t3,a,t50,a,3f6.2)') '(lgtt) Lg travel times', ': ', lg_a, lg_b, lg_min
   if (locmod) write (*,'(t3,a,t50,2a)') '(lmod) local velocity model', ': ', trim(locmodfname)
   write (*,'(t3,a,t50,a,1x,i3)') '(lonr) center of longitude range', ':', longitude_range
   if (lresout) then
      write (*,'(t3,a,t50,a,l1,a,f5.2,a)') '(lres) LRES file output',': ', lresout, ' (LRES = ', lres, ')'
   else
      write (*,'(t3,a,t50,a,l1)') '(lres) LRES file output',': ', lresout
   end if
   write (*,'(t3,a,t50,3(a,f5.2),a)') '(mare) minimum reading errors', ': ', rderr_min_loc, ' (local) /',&
    rderr_min, ' (general) /', rderr_min_depth, ' (depth phases)'
   write (*,'(t3,a,t50,a,l1)') '(noxf) use of readings flagged "x"', ': ', no_xflags
   write (*,'(t3,a,t50,a,l1)') '(nsmd) NEIC station metadata search', ': ', nsmd
   if (oldr_out) then
      write (*,'(t3,a,t50,a,l1,2f7.2)') '(oldr) output limited distance range', ': ', oldr_out, dist1, dist2
   else
      write (*,'(t3,a,t50,a,l1)') '(oldr) output limited distance range', ': ', oldr_out
   end if
   write (*,'(t3,a,t50,a,3f6.2)') '(p_tt) P_ travel times', ': ', p__a, p__b, p__min
   write (*,'(t3,a,t50,a,4f7.2)') '(pert) perturbation', ': ', hlatshift, hlonshift, hdepthshift, htimeshift
   write (*,'(t3,a,t50,a,l1)') '(phid) phase re-identification', ': ', phid
   if (phrp) write (*,'(t3,a,t50,a,10(a,",",1x))') '(phrp) residual plot for phases', ': ', (trim(phrp_phase(i)),i=1,n_phrp)
   write (*,'(t3,a,t50,a,l1)') '(phyp) only P for hypocentroid', ': ', ponly
   write (*,'(t3,a,t50,a,l1)') '(plog) ~.plog file output', ': ', plogout
   if (n_no_phreid .gt. 0) then
      do i = 1,n_no_phreid
         write (*,'(t3,a,t50,2a)') '(ppri) phase renaming prevented for', ': ', no_phreid(i)
      end do
   end if
   write (*,'(t3,a,t50,2a)') '(pttt) perfect theoretical TTs', ': ', pttt_pr
   if (read_ad) then
      do i = 1,n_radf
         write (*,'(t3,a,t50,2a)') '(radf) Read agency and deployment fields for', ': ', radf_stn(i)
      end do
   end if
   if (rdpp) then
      do i = 1,n_rdpp
         write (*,'(t3,a,t50,a)') '(rdpp) relative depth phase plot for ', trim(rdpp_evt(i))
      end do
   end if
   if (rels_set) then
      write (*,'(t3,a,t50,a,3(f5.2,a))') '(rels) reading errors local phases', ': ', rderr_loc_p,&
       ' (P) / ', rderr_loc_s, ' (S) /', rderr_loc_delt, ' (deg)'
   else
      write (*,'(t3,a,t50,a)') '(rels) reading errors local phases', ': not set'
   end if
   write (*,'(t3,a,t50,2a)') '(rfil) reading error file', ': ', trim(rderrfname)
   write (*,'(t3,a,t50,a,3f6.2)') '(rgtt) Rg travel times', ': ', rg_a, rg_b, rg_min
   if (read_hdf) write (*,'(t3,a,t50,2a)') '(rhdf) starting locations', ': ', trim(rhdf_filnam)
   write (*,'(t3,a,t50,a,2f5.2)') '(secv) station elev. corr. vel', ': ', pcrvel, scrvel
   write (*,'(t3,a,t50,a,f6.3,2f4.1)') '(shcl) hypocentroid convergence',': ', cl_epi_h, cl_dep_h, cl_ot_h
   if (n_skip .gt. 0) then
      do i = 1,n_skip
         write (*,'(t3,a,t50,a,1x,a,1x,3(a,1x))') '(skip) skipping station/phase/author', ': ',&
          trim(skip_params(i,1)), trim(skip_params(i,2)), trim(skip_params(i,3))
      end do
   end if
   write (*,'(t3,a,t50,a,l1)') '(splt) seismicity map', ': ', splt
   if (suppstn) then
      do i = 1,n_supp_stn_file
         write (*,'(t3,a,t50,2a)') '(sstn) supplemental station file', ': ', trim(suppfilnam(i))
      end do
   end if
   if (subc_set) then
      write (*,'(t3,a,t50,a,f6.2,2(1x,i3))') '(subc) subcluster selection', ':', subc_delt, subc_nmin, subc_nconnect
   else
      write (*,'(t3,a,t50,a)') '(subc) subcluster selection', ': not set'
   end if
   write (*,'(t3,a,t50,2a)') '(taup) global TT model', ': ', trim(taup_model)
   if (timing_error_correction) then
      do i = 1,n_terr
         write (*,'(t3,a,t50,a,a,f7.2)') '(terr) timing correction for ', ': ', terr_stn(i), terr_corr(i)
      end do
   end if
   write (*,'(t3,a,t50,a,f6.2)') '(tikh) Tikhonov regularization', ': ', tikhonov_factor
   write (*,'(t3,a,t50,a,l1)') '(tlog) tau-p logging', ': ', log_taup
   write (*,'(t3,a,t50,a,2f6.2)') '(tptt) T-phase travel times', ': ', tphase_a, tphase_b
   write (*,'(t3,a,t50,2a)') '(tfil) TT spread file', ': ', trim(ttsprdfname)
   if (tt5e) then
      do i = 1,n_tt5e
         write (*,'(t3,a,t50,a)') '(tt5e) single event tt5 plot for ', trim(tt5e_evt(i))
      end do
   end if
   if (tt5s) then
      do i = 1,n_tt5s
         write (*,'(t3,a,t50,a)') '(tt5s) single station tt5 plot for ', trim(tt5s_sta(i))
      end do
   end if
   if (ttou) then
      do i = 1,n_ttou
         write (*,'(t3,a,t50,a)') '(ttou) empirical TTs for ', trim(ttou_phase(i))
      end do
   end if
   write (*,'(t3,a,t50,a,4l1)') '(vect) vectors plotted', ': ', vectors(1), vectors(2), vectors(3), vectors(4)
   write (*,'(t3,a,t50,a,l1)') '(vlog) verbose logging mode', ': ', verbose_log
   write (*,'(t3,a,t50,a,l1)') '(vscr) verbose screen mode', ': ', verbose_screen
   write (*,'(t3,a,t50,2a)') '(weig) weight residuals', ': ', data_weight_pr
   write (*,'(t3,a,t50,2(a,f4.1))') '(wind) TT window', ': ', wind1, '/', wind2
...
!***********************************************************************************************************************************
subroutine proc_rgtt (command_help)
...
! Rg travel time calculations.
   ...
   if (command_help) then
      write (*,'(3(a/))')&
       '"rgtt" (RG Travel Time) specifies a zero-intercept (sec), slope (sec/degree) and',&
       'minimum epicentral distance (degrees) to be used in calculating travel times for',&
       'Rg. Default values are 0., 36.5 and 2.5.'
      return
   end if
   ...
   if (nvar .eq. 3) then
      ...
      rg_min = value(3)
      if (verbose_screen) then
         write (msg,'(a,3f8.2)') 'proc_rgtt:', rg_a, rg_b, rg_min
         call fyi (trim(msg))
      end if
   else
      call warnings ('proc_rgtt: "rgtt" requires three arguments')
   end if

   return
   
end subroutine proc_rgtt
...
! Starting locations from an HDF file.
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       ...
       '"time", or "depX" issued later in the command file. If any cluster events are missing',&
...
!***********************************************************************************************************************************
subroutine proc_rpth (command_help)

! Determines if ray paths are drawn in the ~_dcal.pdf plot.
! Can be toggled by using the command without an argument, or set explicitly with arguments "on" or "off".
   ...
   if (command_help) then
      write (*,'(6(a/))')&
       '"rpth" (RayPaTH) specifies whether raypaths used in direct calibration will be',&
       'plotted. Skipping the plotting of raypaths speeds up plotting when there are a',&
       'large number of data for diret calibration. Stations used in direct calibration',&
       'are plotted in any case. Issuing the command without an argument toggles the current',&
       'state. The arguments "on" and "off" may be used to set the state explicitly.',&
       'Default is "on".'
      return
   end if
   ...
   if (params(1:1) .eq. ' ') then
      ...
      call warnings ('proc_rpth: illegal argument')
   end if

   if (verbose_screen) then
	  if (rpth) then
		 call fyi ('proc_rpth: direct calibration raypaths will be plotted')
	  else
		 call fyi ('proc_rpth: direct calibration raypaths will not be plotted')
	  end if
   end if
   
   return
   
end subroutine proc_rpth
...
!*****************************************************************************************
subroutine proc_terr (command_help)

! Make a static correction to arrival times of the specified station.
   ...
   if (command_help) then
      write (*,'(8(a/))')&
       '"terr" (timing error) makes a static correction to arrival times at a specified',&
       'station. The correction is applied to all phases. The command takes two arguments,',&
       'the station code and time correction. The command can be issued up to 4 times.'
      return
   end if
   ...
   if (params(1:1) .ne. ' ') then
      ...
      if (nvar .eq. 2) then
         n_terr = n_terr + 1
         if (n_terr .le. n_terr_max) then
            terr_stn(n_terr) = param1(1:5)
            call decode (param2, value, nvar)
            terr_corr(n_terr) = value(1)
         else
            call warnings ('proc_terr: maximum number of instances reached')
            n_terr = n_terr_max
         end if
         if (verbose_screen) then
            call fyi ('proc_terr: station = '//terr_stn(n_terr))
            write (*,'(a,f6.2)') 'proc_terr: time correction  = ', terr_corr(n_terr)
            call fyi (trim(msg))
         end if
      else
         call warnings ('proc_terr: "terr" requires two arguments')
      end if
   else
      call warnings ('proc_terr: "terr" requires two arguments')
   end if
   
   timing_error_correction = (n_terr .gt. 0)
   
   return

end subroutine proc_terr
...
!***********************************************************************************************************************************
subroutine proc_tlog (command_help)

! Logging of the tau-p computations
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       '"tlog" (Tau-p LOG) specifies that details of the tau-p calculations will be written',&
       'to the taup log file (~.taup). This is used for debugging the tau-p code only! The',&
       'output file can exceed 500 MB for moderately large clusters. Issuing the command',&
       'without arguments toggles the current state. The arguments "on" and "off" may be',&
       'used to set the state explicitly. Default is "off".'
      return
   end if
   
   call fyi ('proc_tlog: Warning! This command creates a VERY large output file (100+ MB).')

   if (params(1:1) .eq. ' ') then
      log_taup = .not.log_taup
   else if (params(1:2) .eq. 'on') then
      log_taup = .true.
   else if (params(1:3) .eq. 'off') then
      log_taup = .false.
   else
      call warnings ('proc_tlog: illegal argument')
   end if

   if (log_taup) then
      call fyi ('proc_tlog: tau-p calculations will be logged')
   else
      call fyi ('proc_tlog: tau-p calculations will not be logged')
   end if

   return
   
end subroutine proc_tlog
...
! can be issued multiple times. Supports "all" argument to make the plot for all events that
! have any arrivals within 4°. The plots are put into a subdirectory of the cluster folder.
   ...
   if (command_help) then
      write (*,'(4(a/))')&
       ...
       'The command can be issued multiple times, up to the maximum number of events. If the',&
       'argument "all" is given the plots are made for every event with arrival time data within 4°.'
      return
   end if
   ...
   if (params(1:1) .ne. ' ') then
      if (len_trim(params) .le. 30) then
         ...
         if (trim(params) .eq. 'all') then
            tt5e_all = .true.
            if (verbose_screen) call fyi ('proc_tt5e: tt5e plots will be done for all events')
         else
            n_tt5e = n_tt5e + 1
			tt5e_evt(n_tt5e) = trim(params)
            if (verbose_screen) call fyi ('proc_tt5e: tt5e plot for event '//trim(params))
         end if
      else
      ...
      call warnings ('proc_tt5e: "tt5e" requires one argument, an event name, or the argument "all"')
   end if
...
! Verbose_screen mode, extra output to standard output
   ...
   verbose_taup = verbose_screen ! Print extra information from tau-p calculations
...
! Plotting of cross-sections (depth profiles)
   ...
   if (command_help) then
      write (*,'(10(a/))')&
       '"xsec" (X-SECtion) controls the plotting of cross-sections through the hypocentroid.',&
       'The command takes one argument, the azimuth of projection measured clockwise',&
       'from north. A value of 90° projects hypocenters onto a plane of constant longitude.',&
       'Uncertainty in depth is now plotted (when available). Events with depth constraint',&
       'of some kind are plotted as open circles. Free-depth relocations are distinguished',&
       'by being in red. In fixed-depth relocations, events with depth constraint are',&
       'plotted in open black circles. Events with no depth constraint (depth codes "c", "i"',&
       'and "u") are plotted as "x", cluster default "c" in black, the other two in blue.',&
       'Event numbers and user-defined stars are plotted. Multiple cross-sections can be,',&
       'defined up the limit set by the parameter "n_xsec_max" (currently 4)'
      return
   end if
   ...
   if (nvar .eq. 1) then
      if (n_xsec .lt. n_xsec_max) then
         n_xsec = n_xsec + 1
         xsec_az(n_xsec) = value(1)
		 if (verbose_screen) then
			write (msg,'(a,i1,1x,f5.1)') 'proc_xsec: ', n_xsec, xsec_az(n_xsec)
			call fyi (trim(msg))
		 end if
      else
      ...
      call warnings ('proc_xsec: "xsec" requires one argument')
   end if
