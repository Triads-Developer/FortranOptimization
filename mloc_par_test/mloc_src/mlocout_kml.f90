...
! of calibrated clusters. By itself, this entry is not a legal kml file.
   ...
   integer :: i, it, it1, lenb, io_commentary, ios, lunit
   real :: lon_test
   character(len=132) :: msg, line132
   character(len=100) :: cluster_name, commentary_path, outfil
   ...
   open (io_gccel,file=outfil,status='new')
   ...
   if (ex) then
      ...
      call warnings ('gccel_hypocentroid: commentary file not found')
   end if
   do i=1,9
...
! epicenters of calibrated clusters.
   ...
   inquire (unit=io_gccel,opened=op)
   if (.not.op) then
      msg = 'gccel_epicenters: file '//trim(outfil)//' was not opened'
      call oops (trim(msg))
   end if
...
subroutine write_epicenter_kml (unit, ev_name, hypocenter)
   ...
   integer :: unit
   real :: scale, scale0, scale_factor, xmag
   character(len=116) :: ev_name
   character(len=80) :: hypocenter
   character(len=22) :: ot
   character(len=9) :: cxlon
   character(len=8) :: cxlat
