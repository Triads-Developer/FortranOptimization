...
!      print *, 'j jidx indx jndx =', j, jidx(j), indx(i,2), jndx(j,2)
   ...
   if (log_taup) then
      ...
      write (io_taup_log,*) 'depset: msrc isrc odep zs us', msrc, isrc, odep, sngl(zs), sngl(us(1)), sngl(us(2))
...
!***********************************************************************************************************************************      
 ...
 11   do i=1,ku(nph)
      ...
      if (log_taup) write (io_taup_log,*) 'depcor: bkin', ks, sngl(sgn), sngl(tauu(1,nph)), sngl(xu(1,nph))
 ...
 8    msrc(nph) = ks
      ...
      do k = 1,k1
         ...
         if (log_taup) write (io_taup_log,*) 'depcor: up x:  k mu', k, mu, sngl(xu(mu,nph)), sngl(xc(mu))
         mu = mu + 1
      end do
      go to 39
 ...
 39   xus1(nph) = 0d0
      ...
      tauus1(nph) = tauc(k1)
      xus1(nph) = xc(mu)
      if (log_taup) write (io_taup_log,*) 'depcor: is ks tauus1 xus1', is, ks, sngl(tauus1(nph)), sngl(xus1(nph)), '  *'
      go to 33
 ...
 33   if (pm(is+1,nph) .lt. umin) go to 41
      ...
      if (log_taup) write (io_taup_log,*) 'depcor: is ks tauus2 xus2', is, ks, sngl(tauus2(nph)), sngl(xus2(nph))
 ...
 41   iph = mod(nph,2) + 1
      ...
      do j = 1,nbrn
         ...
         if (log_taup) write (io_taup_log,*) 'depcor: j phcd px umin =', j, ' ', phcd(j), px(j,1), px(j,2), umin
 ...
 47   z1 = zmod(umin,j,iph)
      ...
      tauus1(iph) = tauus1(iph) + ttau
      xus1(iph) = xus1(iph) + tx
      if (log_taup) write (io_taup_log,*) 'depcor: is ks tauusp xusp', j, ks, sngl(tauus1(iph)), sngl(xus1(iph))
...
!     if (odep .ge. deplim .or. odep .le. .1) go to 43
 ...
 58   du = amin1(1e-5+(odep-.4)*2e-5,1e-5)
      if (log_taup) write (io_taup_log,*) 'depcor: Add:  nph is ka odep du us =', nph, is, ka, odep, sngl(du), sngl(us(nph))
      lp = lpower
      k = 0
      do l = ka,1,-1
         k = k + 1
         ...
         if (log_taup) write (io_taup_log,*) 'depcor: l k ua taua', l, k, sngl(ua(k,nph)), sngl(taua(k,nph))
 ...
 43   j = 1
      ...
      do i = 1,nseg
         if (.not.segmsk(i)) cycle
         if (log_taup) write (io_taup_log,*) 'depcor: i iidx nafl nph', i, iidx(i), nafl(i,1), nph
         if (iidx(i) .gt. 0 .or. iabs(nafl(i,1)) .ne. nph .or. (msrc(nph) .le. 0 .and. nafl(i,1) .gt. 0)) cycle
    ...
    22   if (log_taup) write (io_taup_log,*) 'depcor: k m', k, m
         ...
         if (log_taup) write (io_taup_log,*) 'depcor: nph iph kph tauus1 tauus2 tau =', nph, iph, kph, sngl(tauus1(1)),&
          sngl(tauus1(2)), sngl(tauus2(1)), sngl(tauus2(2)), sngl(tau(1,k))
    24   m = 1
    ...
    37   if (log_taup) write (io_taup_log,*) 'depcor: j jndx jidx', j, jndx(j,1), jndx(j,2), jidx(j), ' ', phcd(j)
         do l = 1,2
    ...
    29      xbrn(j,l) = fac*(xus1(iph)+xus2(iph)+xus1(kph)+xus2(kph))+sgn*xus1(nph)
            if (log_taup) then
               ...
               write (io_taup_log,*) 'depcor: nph iph kph xus1 xus2 xbrn =', nph, iph, kph, sngl(xus1(1)), sngl(xus1(2)),&
                sngl(xus2(1)), sngl(xus2(2)), sngl(xbrn(j,l))
            end if
         end do
         if (j .ge. nbrn) cycle
         j = j + 1
...
!     logical log
      ...
      i1 = jndx(jb,1)
      i2 = jndx(jb,2)
      if (log_taup) write (io_taup_log,*) 'spfit: jb i1 i2 pt =', jb, i1, i2, sngl(pt(i1)), sngl(pt(i2))
...
!     xmin=xmin*xn
 ...
 12   nn = i2 - i1 + 1
      ...
      if (log_taup) write (io_taup_log,301,iostat=ios) jb, k, nn, int, newgrd, makgrd, xbrn(jb,1),&
       xbrn(jb,2), (i,pt(i-1+i1), tau(1,i-1+i1), (tcoef(j,i,k),j=1,5),i=1,nn)
...
!     call appx(i1,i2,xbrn(jb,1),xbrn(jb,2))
 ...
 102  format ()
      ...
      dbrn(jb,1) = dmx
      dbrn(jb,2) = dbrnch
      if (log_taup) write (io_taup_log,101,iostat=ios) phcd(jb), (jndx(jb,k),k=1,2), (cn*xbrn(jb,k),k=1,2), xbrn(jb,3),&
       (cn*dbrn(jb,k),k=1,2), (idel(jb,k),k=1,3), int, newgrd, makgrd
 101  format (1x,a,2i5,2f8.2,f8.4,2f8.2,4i3,2l2)
      go to 15
      
 3    if (log_taup) write (io_taup_log,103,iostat=ios) phcd(jb), (jndx(jb,k),k=1,2), (cn*xbrn(jb,k),k=1,2), xbrn(jb,3),&
...
!        print *,'jp phcd xbrn',jp,'  ',phcd(jp),xbrn(jp,1)
         if ((phcd(jp) .eq. 'Pg' .or. phcd(jp) .eq. 'Pb' .or. phcd(jp) .eq. 'Pn' .or. phcd(jp) .eq. 'P')&
          .and. xbrn(jp,1) .le. 0d0) go to 3
      end do
      jp = 0
 ...
 3    do js = mbr1,mbr2
   !     print *,'js phcd xbrn',js,'  ',phcd(js),xbrn(js,1)
         if ((phcd(js) .eq. 'Sg' .or. phcd(js) .eq. 'Sb' .or. phcd(js) .eq. 'Sn' .or. phcd(js) .eq. 'S')&
          .and. xbrn(js,1) .le. 0d0) go to 1
      end do
      js=0
