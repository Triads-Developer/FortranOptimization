...
!       write (io_log,'(a)') ' iev   j mnf Con  FH  FC  nt  jb qcname(jb)             vnhat  dthat'
      do iev = 1,nev
         do j = 1,nst(iev)
            if (connected(iev,j)) then
               ...
               ! This was for some special testing. Don't uncomment unless you know what you're doing!
! 			   write (io_log,'(3i4,3x,l1,3x,l1,3x,l1,2i4,1x,a,1x,f6.3,1x f6.3)') &
...
!         end do
      ...
      k = 0
      ! This was for some special testing. Don't uncomment unless you know what you're doing!
!       write (io_log,'(a)') ' iev   j mnf   k c qbdthat(k)  qbadx(k)       eci'
      do iev = 1,nev
         do j = 1,nst(iev)
            if (connected(iev,j)) then
               k = k + 1
      ...
      ! This was for some special testing. Don't uncomment unless you know what you're doing!
!                write (io_log,'(4i4,1x,l1,1x,3f10.3)') iev, j, mnf_line(iev,j), k,&
...
!    write (*,*) size(ahat,1), size(ahat,2)
   ...
   write (io_log,'(t7,a,i5,a)') ' NT  = ', nt, ' (# data)'
   write (io_log,'(t7,a,i5,a)') ' MTH = ', mth, ' (# free parameters)'
   write (io_log,'(t7,a,i5,a)') ' NQ  = ', nq, ' (# indep. station-phases)'
