...
! If 'iev' is greater than zero then the subroutine expects to read a standard v1.3.N MNF
...
! 2023/6/21: All MNF 1.3 family formats supported, but not earlier ones
   ...
   if (format_line(1:1) .eq. 'F') then
      ...
      read (format_line(10:12),'(f3.1)') fv
      if (fv .lt. 1.3) then
         write (msg,'(a)') 'read_mnf: MNF version '//format_line(1:3)//' is not supported'
         call oops (trim(msg))
      end if
      
      ! Event data file
      if (format_version(1:3) .eq. '1.3') then
	  ...
	  ! Differential time file
	  if (format_version(1:3) .eq. '1.5') then
	     call read_mnf_15 ()
      else
         if (iev .eq. 0) then
            write (msg,'(4a)') 'read_mnf: unsupported format version ',&
             format_version, ' for', diffdatfilnam
         else
            write (msg,'(3a,i3,1x,a)') 'read_mnf: unsupported format version ',&
             format_version, ' for event ', iev, evtnam(iev)
         end if
         call oops (trim(msg))
      end if
      ...
      if (iev .eq. 0) then
         ...
         write (msg,'(a,i3,1x,a)') 'read_mnf: no MNF format record found for event ',&
          iev, evtnam(iev)
      end if
      call oops (trim(msg))
   end if
...
!*****************************************************************************************
subroutine read_mnf_134 (iev, inn)

! Read input data files in MNF (mloc native format) v1.3.4.
   ...
   integer :: ipah_corr, ipam_corr
   integer :: elev_ifsm
   integer :: jdate, julday
!   integer :: scale_length
   real :: p_arrtime, hms2s, smin_in, smaj_in, sece_sigma, res_test, pas_corr
   real :: time_uncorrected, time_corrected
!   real :: t11, t12, t22, 
   real :: usrc(2)
   real :: lat_ifsm, lon_ifsm
   logical :: h_set, m_set, i_set, skip_station, skip_phase, skip_author, depth_constraint,&
    pass, bdp, radf_stname, phase_ok, date_range, matched
   character(len=1) :: depth_inp_c_dr
   character(len=5) :: psta, original_code, data_author, data_reporter, agency_ifsm
   character(len=7) :: ds_test
   character(len=12) :: dslc_test
   ...
   do
      ...
      if (ios .lt. 0)  call oops ('read_mnf_134: EOF reached unexpectedly')
      ...
      if (linein(1:1) .eq. 'E')  then
         if (linein(3:3) .eq. '-') then
            write (msg,'(a,i3,1x,a)') 'read_mnf_134: No phase data for event ',&
             iev, trim(evtnam(iev))
            call oops (trim(msg))
         end if
         ...
         if (event_comment(iev) .ne. ' ') then
            if (annotation(iev) .ne. ' ') then
               write (msg,'(a,i3,1x,2a)') 'read_mnf_134: event comment for event ',&
                iev, trim(evtnam(iev)), ' is being over-written by ANNO command'
               if (verbose_screen) call fyi (trim(msg))
            else
               annotation(iev) = event_comment(iev)(1:20)
               write (msg,'(a,i3,1x,4a)') 'read_mnf_134: annotation for event ',&
                iev, trim(evtnam(iev)), ' read from input file: "', trim(annotation(iev)), '"'
               if (verbose_screen) call fyi (trim(msg))
            end if
         end if
      end if
      
      ! Event ID record (max 10 characters are kept)
      if (linein(1:1) .eq. 'I') then
         if (linein(3:3) .eq. '=' .or. .not.i_set) then
			...
			if (j .gt. 22) then
			   ...
			   evid(iev) = linein(13:22)
			   i_set = .true.
			else if (j .lt. 22 .and. j .ge. 12) then
      ...
      if (linein(1:1) .eq. 'H') then
         if (linein(3:3) .eq. '=' .or. .not.h_set) then
            h_set = .true.
            if (verbose_screen) then
               write (msg,'(2a)') 'read_mnf_134: ', trim(linein)
               call fyi (trim(msg))
            end if
            ...
            if (linein(90:91) .eq. 'GT') then
               write (msg,'(a,i3,1x,4a)') 'read_mnf_134: event ', iev, trim(evtnam(iev)),&
      ...
      if (linein(1:1) .eq. 'P') then
         if (.not.phase_ok(linein(24:31))) then  ! Check if this is a junk phase
            write (msg,'(a,i5)') 'read_mnf_134: junk phase: '//linein(24:31)//' skipped at line ', line_number
            if (verbose_screen) call fyi (trim(msg))
         ...
         diff_line(iev,ird) = 0
         if (ird .eq. ntmax0) call oops ('read_mnf_134: ird exceeds ntmax0')
         nst(iev) = ird
         ...
         if (pas(iev,ird) .gt. 59.999) then
            write (msg,'(a,f6.3,a,i5)') 'read_mnf_134: time bug, arrival time seconds field : ',&
             pas(iev,ird), ' at line ', line_number
         ...
         if (read_ad .and. radf_stname(stname(iev,ird))) then
			agency(iev,ird) = linein(75:79)
			deployment(iev,ird) = linein(81:88)
			location(iev,ird) = linein(96:97)
			channel(iev,ird) = linein(99:101)
			adslcaa(iev,ird)(1:51) = linein(75:125)
			data_author = linein(103:107) ! not used
			data_reporter = linein(109:113)
			readsrc(iev,ird) = data_reporter//'   '
		 else   
			agency(iev,ird) = ' '
			deployment(iev,ird) = ' '
			adslcaa(iev,ird)(1:51) = linein(75:125)
			channel(iev,ird) = linein(99:101)
			readsrc(iev,ird) = linein(109:113)
		 end if
         sad(iev,ird) = stname(iev,ird)//' '//agency(iev,ird)//' '//deployment(iev,ird)
         
         ! Ignore 'x' flags
         if (no_xflags) then
            if (fcode(iev,ird) .eq. 'x') then
               fcode(iev,ird) = ' '
            end if
         end if
         ...
         if (original_code .ne. '     ' .and. original_code .ne. stname(iev,ird)) then
            if (verbose_screen) call fyi ('read_mnf_134: '//trim(original_code)//' changed to '//stname(iev,ird))
            if (verbose_log) write (io_stn_log,'(i3,1x,5a)') iev, trim(evtnam(iev)), ': ',&
         ...
         if (timing_error_correction) then
            do i = 1,n_terr
               if (stname(iev,ird) .eq. terr_stn(i)) then
                  time_uncorrected = hms2s(ipah(iev,ird), ipam(iev,ird), pas(iev,ird))
                  time_corrected = time_uncorrected + terr_corr(i)
                  if (time_corrected .gt. 86400.) time_corrected = time_corrected - 86400.
                  if (time_corrected .lt. 0.) time_corrected = time_corrected + 86400.
                  call timecr (time_corrected, ipah_corr, ipam_corr, pas_corr)
                  write (msg,'(2a,1x,a,1x,2i2,a,f4.1,a,2i2,a,f4.1)') 'read_mnf_134: ',&
                   stname(iev,ird), phase0(iev,ird),&
                   ipah(iev,ird), ipam(iev,ird), ':', pas(iev,ird), ' corrected to ',&
                   ipah_corr, ipam_corr, ':', pas_corr
                  call fyi (trim(msg))
                  ipah(iev,ird) = ipah_corr
                  ipam(iev,ird) = ipam_corr
                  pas(iev,ird) = pas_corr
                  exit
               end if
            end do
         end if
         ...
         if (fcode(iev,ird) .eq. ' ') then
            ...
            if (res_test .ge. 55. .and. res_test .le. 65.) then
               write (msg,'(a,i5)') 'read_mnf_134: possible one-minute error for line ', mnf_line(iev,ird)
               call warnings (trim(msg))
            end if
         end if
         ...
         ! Generate a supplemental station file entry from the NEIC station metadata file.
         ! The match, which causes a new entry in the supplemental station file, is based on
         ! "deployment/station code" from the MNF data file, and the date of the event must
         ! be within the operational date range of the entry in NEIC station metadata file.
         if (nsmd) then
            if (linein(81:82) .ne. '  ') then ! Don't bother checking if there is no network/deployment code in the phase record
            
               dslc_test = linein(81:82)//linein(90:94)//linein(96:97)//linein(99:101)
               ! Julian date for current event
               call juldat (ipay(iev,ird), ipamo(iev,ird), ipad(iev,ird), julday, 0)
               jdate = ipay(iev,ird)*1000 + julday
               if (verbose_log) write (io_log,'(2i4,1x,a,1x,i8)') iev, ird, dslc_test, jdate
         ...
         if (kcode(iev,ird) .eq. -1 .and. ifsm) then ! kcode = -1 for stations not found in the current station list
            if (linein(81:82) .ne. '  ') then ! Don't bother checking if there is no network/deployment code in the phase record
               ...
               ds_test = linein(81:82)//linein(90:94)
               ! Julian date for current event
               call juldat (ipay(iev,ird), ipamo(iev,ird), ipad(iev,ird), julday, 0)
               jdate = ipay(iev,ird)*1000 + julday
         ...
         ! Skip depth phases (flag 'p') at short epicentral distances, and also from stations listed in the
         ! bogus depth station list
         if (rel_depth_phase(iev,ird) .and. pass(fcode(iev,ird))) then
            if (delt(iev,ird) .lt. 26.0 .or. bdp(iev,ird)) fcode(iev,ird) = 'p'
         end if
      ...
      ! Every line read should have a record identifier in column 1
      if (linein(1:1) .eq. ' ') call oops ('read_mnf_134: Record flag missing! '//linein)
      
   end do
   
   write (msg,'(3x,a,i6,a)') 'read_mnf_134: ', nphreid, ' phases re-identified for iteration 0'
   if (verbose_screen) call fyi (trim(msg))
   write (io_log, '(a)') trim(msg)
   
   return
   
end subroutine read_mnf_134


!*****************************************************************************************
subroutine read_mnf_13 (iev, inn, format_version)

! Read input data files in MNF (mloc native format) format v1.3.3 and earlier
...
!   real :: t11, t12, t22, 
   ...
   do
      ...
      if (linein(1:1) .eq. 'P') then
         if (.not.phase_ok(linein(24:31))) then  ! Check if this is a junk phase
            write (msg,'(a,i5)') 'read_mnf_13: junk phase: '//linein(24:31)//' skipped at line ', line_number
            if (verbose_screen) call fyi (trim(msg))
            if (verbose_log) write (io_log,'(a)') trim(msg)
            cycle
         end if
         ...
         if (read_ad .and. radf_stname(stname(iev,ird))) then
            if (linein(103:105) .eq. 'ISC' .and. linein(75:88) .eq. '     .        ') then
			   agency(iev,ird) = 'ISC  '
			   deployment(iev,ird) = 'IR      '
            else   
			   agency(iev,ird) = linein(75:79)
			   deployment(iev,ird) = linein(81:88)
			end if
         ...
         if (timing_error_correction) then
            do i = 1,n_terr
               if (stname(iev,ird) .eq. terr_stn(i)) then
                  ...
                  if (time_corrected .lt. 0.) time_corrected = time_corrected + 86400.
                  call timecr (time_corrected, ipah_corr, ipam_corr, pas_corr)
                  write (msg,'(2a,1x,a,1x,2i2,a,f4.1,a,2i2,a,f4.1)') 'read_mnf_13: ',&
                   stname(iev,ird), phase0(iev,ird),&
                   ipah(iev,ird), ipam(iev,ird), ':', pas(iev,ird), ' corrected to ',&
                   ipah_corr, ipam_corr, ':', pas_corr
                  call fyi (trim(msg))
         ...
         ! Generate a supplemental station file entry from the NEIC station metadate file.
         ! The match, which causes a new entry in the supplemental station file, is based on
         ! "deployment/station code/location/channel" from the MNF data file. When coordinates
         ! are actually assigned at a later step, only station code and deployment are used
         ! to match so there can be what are considered duplicate entries. These are noted by
         ! an FYI message during runtime. These warnings can usually be ignored.
         if (nsmd) then
            if (linein(81:82) .ne. '  ') then ! Don't bother checking if there is no network/deployment code in the phase record
            
               dslc_test = linein(81:82)//linein(5:9)//linein(96:97)//linein(99:101)
               ! Julian date for current event
               call juldat (ipay(iev,ird), ipamo(iev,ird), ipad(iev,ird), julday, 0)
               jdate = ipay(iev,ird)*1000 + julday
               if (verbose_log) write (io_log,'(2i4,1x,a,1x,i8)') iev, ird, dslc_test, jdate
               ...
               if (n_nsmd_written .ge. 1) then
                  do i = 1,n_nsmd_written
                     if (dslc_test .ne. dslc_matched(i)) cycle ! Network/deployment code
                     if (date_range(jdate,nsmd_on_matched(i),nsmd_off_matched(i))) then
                        matched = .true.
                        exit
                     end if
               ...
               if (.not.matched) then
                  if (n_nsmd_written .lt. n_nsmd_matched_max) then
                     do i = 1, n_nsmd_read
                        if (dslc_test .eq. dslc(i)) then
						   if (date_range(jdate,nsmd_on(i),nsmd_off(i))) then
							  n_nsmd_written = n_nsmd_written + 1
							  write (io_nsmd,'(a)') nsmd_line(i)
							  dslc_matched(n_nsmd_written) = dslc(i)
							  nsmd_on_matched(n_nsmd_written) = nsmd_on(i)
							  nsmd_off_matched(n_nsmd_written) = nsmd_off(i)
							  exit
						   end if
                     ...
                     write (msg,'(a,i4,a)') 'read_mnf_13: maximum number of matched metadata entries (',&
                      n_nsmd_matched_max, ') written'
                     call warnings (trim(msg))
                  end if
               end if
         ...
         ! Optionally skip readings with blank phase codes
      ...
      ! Every line read should have a record identifier in column 1
      if (linein(1:1) .eq. ' ') call oops ('read_mnf_13: Record flag missing! '//linein)
...
!*****************************************************************************************
subroutine write_mnf_134 (io_unit, iev, it)

! Write one event in MNF v1.3.4 format.
...
!   integer :: scale_length
   ...
   ! Some lines are copied directly from the input file
   open (io_in,file=infile(iev),status='old')

   ! Event record (one required)
   ! An event file could not be read if it lacks an event record
   do
      read (io_in,'(a)') linein
      if (linein(1:1) .eq. '#') cycle
      if (linein(1:2) .eq. 'E ') then
         write (io_unit,'(a)') linein
         exit
      end if
   end do
   
   ! Event ID lines (optional but possibly multiple instances)
   ! Search ends at first hypocenter record
   do
      read (io_in,'(a)') linein
      if (linein(1:1) .eq. '#') cycle
      if (linein(1:2) .eq. 'I ') then
         write (io_unit,'(a)') linein
      end if
      if (linein(1:2) .eq. 'H ') then ! No more event ID records expected after the first hypocenter record
         backspace (io_in)
         exit
      end if
   end do
   
   ! Write new hypocenter record from the latest run
   ...
   ! Hypocenter line
   fmt = '(2a,1x,i4,a,i2.2,a,2(i2.2,1x),i2.2,2f6.2,2f10.4,1x,i3,2f6.2,f6.1,1x,a1,2(1x,a5),1x,a4,1x,a8,1x,a,t125,a)'
   if (calibration) then
   ...
   ! Retrieve previous hypocenter records (one required, possibly multiple)
   ! File is already open and positioned after the event record or the last event ID record
   do
      read (io_in,'(a)') linein
      if (linein(1:1) .eq. '#') cycle
      if (linein(1:2) .eq. 'H ') then
         if (linein(3:3) .eq. '=') linein(3:3) = ' '
         write (io_unit,'(a)') linein
      end if
      if (linein(1:2) .eq. 'M ' .or. linein(1:2) .eq. 'D ' .or. linein(1:2) .eq. 'P ') then
         backspace (io_in)
         exit
      end if
   end do

   ! Retrieve previous magnitude and depth records (optional)
   ! File is already open and positioned after the event record or the last hypocenter record
   do
      read (io_in,'(a)') linein
      if (linein(1:1) .eq. '#') cycle
      if (linein(1:2) .eq. 'M ') write (io_unit,'(a)') linein
      if (linein(1:2) .eq. 'D ') write (io_unit,'(a)') linein
   ...
   ! Phase lines
   fmt0 = '(a1,1x,a1,1x,a6,1x,a6,1x,a3,1x,a1,a8,1x,i4,4(1x,i2),1x,f3.0,4x,i2,1x,a5,1x,a8,1x,a51)'
   fmt1 = '(a1,1x,a1,1x,a6,1x,a6,1x,a3,1x,a1,a8,1x,i4,4(1x,i2),1x,f4.1,3x,i2,1x,a5,1x,a8,1x,a51)'
   fmt2 = '(a1,1x,a1,1x,a6,1x,a6,1x,a3,1x,a1,a8,1x,i4,4(1x,i2),1x,f5.2,2x,i2,1x,a5,1x,a8,1x,a51)'
   fmt3 = '(a1,1x,a1,1x,a6,1x,a6,1x,a3,1x,a1,a8,1x,i4,4(1x,i2),1x,f6.3,1x,i2,1x,a5,1x,a8,1x,a51)'
   ...
   do i = 1,nst(iev)
      ...
      write (io_unit,fmt)&
       ...
       dts_pr,& ! a5
       phase0(iev,ird),& ! a8
       adslcaa(iev,ird) ! a51 or possibly less if an earlier format was read
   end do
   
   ! Stop line
   write (io_unit,'(a,t125,a)') 'STOP', ' '
   
   return
   
end subroutine write_mnf_134
...
!   real :: hms2s
   ...
   if (lat_cf(iev) .ge. -90.) then ! Command file
      latp(iev,0) = lat_cf(iev)
   else if (lat_hdf(iev) .ge. -90.) then ! HDF file
      latp(iev,0) = lat_hdf(iev)
   else ! Input file
      latp(iev,0) = lat_inp(iev)
   end if
   
   ! Starting longitude
   if (lon_cf(iev) .ge. -180.) then ! Command file
      lonp(iev,0) = lon_cf(iev)
   else if (lon_hdf(iev) .ge. -180.) then ! HDF file
      lonp(iev,0) = lon_hdf(iev)
   else ! Input file
      lonp(iev,0) = lon_inp(iev)
   end if
...
!       depset_pr(iev) = depth_inp_c(iev)
   ...
   ! Perturbations (PERT command)
   latp(iev,0) = latp(iev,0) + hlatshift
   lonp(iev,0) = lonp(iev,0) + hlonshift
   depthp(iev,0) = depthp(iev,0) + hdepthshift
   time0 = time0 + htimeshift
   
   ! Convert origin time to hour-minutes-seconds
   ...
   ! This is very unlikely unless the user has reduced the parameter n_hrdp_max
   if (depthp(iev,0) .gt. n_hrdp_max-1) then
      write (msg,'(a,i3,2(a,f6.1),a)') 'starting_hypocenter: starting depth for event ', iev, ' (', depthp(iev,0),&
       ') greater than the range for depth tests (', n_hrdp_max-1, ')' 
      call warnings (trim(msg))
   end if
