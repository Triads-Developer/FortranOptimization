...
! use of the "NSMD" command, many duplicates may be noted. This happens
...
!    and a new code for the new location has been assigned retroactively.
   ...
   ! These checks are only made for stations from the master list, not supplemental station files.
!    if (suppstn) then
!       if (kcode(iev,ird) .le. n_supp_stn) return
!    end if
