...
! 6/29/2018: Fixed a bug reading the phase_data file when differential time data are used
   ...
   integer iter, i, j, n, iev, idiff0, idiff, ios, nres(ndiffmax), dot, iev_idiff(ndiffmax,2) 
   real data(1000), sum, mean, sn, dts, res(ndiffmax,4), resout, xmean
   ...
   if (diff) then
      nres = 0
      res = 0.
      write (*,'(/a)') 'IDIFF IEV  RAWRES    MEAN     RES'
      do
         ...
         if (charin(162:165) .ne. '****' .and. charin(162:165) .ne. '    ' .and. charin(162:165) .ne. 'IDIF') then
            ...
            if (idiff .gt. 0 .and. idiff .le. ndiffmax) then
               ...
               if (nres(idiff) .eq. 2) then
                  ...
                  write (*,'(/1x,2i4,f8.2,2f8.3,2x,a)') idiff, iev_idiff(idiff,1), res(idiff,1), xmean, res(idiff,3), charin(1:26)
                  write (*,'(5x,i4,f8.2,2f8.3,2x,a)')       iev_idiff(idiff,2), res(idiff,2), xmean, res(idiff,4), charin(1:26) 
               end if
            end if
         end if
      end do
