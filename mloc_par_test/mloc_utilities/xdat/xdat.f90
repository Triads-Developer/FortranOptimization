...
! Added the option to skip flagging of teleseismic depth phases 10/15/2020.
   ...
   write (*,'(a/)') 'Release date October 15, 2020'
   ...
   do
      ...
      if (.not.depth_phases) then
         if (phase0(1:2) .eq. 'pP') cycle
         if (phase0(1:2) .eq. 'sP') cycle
         if (phase0(1:3) .eq. 'pwP') cycle
      end if
