...
! 2023/03/21: Adapted for use with MNF v1.3.4 only. No conversion from earlier formats.
   ...
   character(len=125) :: linein, buffer(n_line_max)
   ...
   logical :: cfil, preferred, ex, n_select, epicenter_pass, preferred_depth,&
    bulletin, depth_pass, nearest_pass, magnitude_pass, use_epicenter, use_depth,&
   ...
   buffer = ' '
   n_selected = 0
   current_version = '1.3.4 ' ! Current version of the MNF format
   evid_source = ' '
   use_epicenter = .false.
   use_depth = .false.
   use_nearest = .false.
   ...
   write (*,'(a/)') 'Release date March 21, 2023'
   ...
   do
      ...
      if (linein(1:1) .eq. 'F') then ! Check if the Bulletin uses the current MNF format
         if (linein(10:14) .ne. '1.3.4') then
            write (*,'(a)') 'Unsupported format: '//linein(10:14)
            stop
         end if
   ...
   if (bulletin) then
      ...
      write (2,'(a1,t5,a,t125,a)') 'B', trim(comment), ' '
      write (2,'(a,t125,a)') format_line, ' '
   end if
   do
      read (1,'(a)',iostat=ios) linein
      ...
      if (linein(1:4) .eq. 'STOP') then ! Dump buffer
         if (n_select .and. (np .ge. npmin)) then
            ...
            if (preferred) then
               ...
               if (.not.bulletin) write (2,'(a,t125,a)') format_line, ' '
               do i = 1,line
                  write (2,'(a)') buffer(i)
               end do
