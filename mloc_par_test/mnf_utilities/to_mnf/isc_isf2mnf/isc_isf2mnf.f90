...
! Reads a file containing arrival time data for one or more events in the ISF format returned by the ISC Bulletin web search
! when the options for header information and comments are selected.
! This code expects the input to be in ISF2.1 format.
! The output is in MNF (mloc native format) format, v1.3.4.
...
! filename, is the presence of a bulletin record at the top of the file.
   ...
   logical :: event_on, bulletin, ex, cfil, phase_ok, at, event_written, preferred_hypo, format_check
   integer :: ios, nev, phase_seconds_int, iblank, int2
   real :: azeq, smin, smaj
 
   version = '1.3.4 ' ! MNF version
   nev = 0
   event_on = .false.
   cfil = .false.
   preferred_hypo = .false.
   format_check = .false. ! Used to make sure ISF2.1 format is read
   
   write (*,'(a/)') 'Release date March 20, 2023, writing MNF v'//version
   ...
   do ! loop over all lines
      ...
      if (linein(1:31) .eq. 'DATA_TYPE BULLETIN ISF2.1:short') then
         format_check = .true.
         write (*,'(a)') 'ISF2.1 format confirmed'
         cycle
      end if
      ...
      if (linein(1:4) .eq. 'STOP') then
         close (1)
         if (bulletin) then
            write (2,'(a,t125,a)') 'STOP', ' '
            write (2,'(a,t125,a)') 'EOF', ' '
            close (2)
         end if
         if (cfil) close (3)
      ...
      if (linein(1:6) .eq. 'Event ') then
         if (.not.format_check) then
            write (*,'()') 'Error: ISF2.1 format not confirmed, processing aborted'
            close (1)
            close (2)
            stop
         end if
...
!         end if
         hypo_orid = linein(129:139)
         hypo_author = linein(119:127)
...
!         end if
      ...
      if (linein(1:9) .eq. 'Magnitude') then ! Magnitude lines
         ...
         if (linein(1:1) .ne. ' ') then
            read (linein(7:10),'(f4.1)') magnitude
            magnitude_scale = linein(1:5)
            magnitude_usage = ' '
            magnitude_author = linein(21:29)
            magnitude_orid = linein(31:41)
      ...
      if (linein(1:4) .eq. 'Sta ') then ! Phase readings
         do
            ...
            location = '  '
            arrid_pr = linein(115:125)
            agency = linein(127:131)
            deployment = linein(133:140)
            location = linein(142:143)
            data_author = linein(145:149)
            data_reporter = linein(151:155)
            channel = linein(157:159)
            phase_usage = ' '
            station_flag = ' '
            phase_flag = ' '
...
! Write an MNF bulletin line
   ...
   write (io_unit,'(2a,t125,a)') 'B   ', trim(comment), ' '
...
! Write an MNF format line
   ...
   write (io_unit,'(2a,t125,a)') 'F   MNF v', version, ' '
...
! Write an MNF event line
   ...
   write (io_unit,'(a1,1x,a1,1x,a,t125,a)') 'E', event_usage, trim(event_anno), ' '
...
! Write an MNF event ID line
   ...
   write (io_unit,'(a,t5,a,t12,a,t125,a)') 'I', 'ISC', trim(evid), ' '
...
! Write an MNF hypocenter line
   ...
   fmt = '(2a,1x,i4,a,i2.2,a,i2.2,1x,i2.2,1x,i2.2,1x,f5.2,1x,5x,2f10.4,1x,a3,2(1x,a5),f6.1,t95,a9,t115,a11)'
...
! Write an MNF magnitude line
   ...
   write (io_unit,'(2a,1x,f3.1,2x,a5,1x,a9,t115,a11)') 'M ', magnitude_usage, magnitude,&
...
! Write an MNF phase line
   ...
   character(len=140) :: fmt
   character(len=27) :: adslc
   character(len=6) :: phase_seconds_pr
   
   adslc = agency//'.'//deployment//'.'//station(1:5)//'.'//location//'.'//channel
   
   fmt = '(a1,1x,a1,a1,a6,1x,a6,1x,a3,1x,a1,a8,1x,i4,1x,4(i2,1x),a6,1x,a2,1x,a5,1x,a8,1x,a27,1x,a5,1x,a5,1x,a11)'
...
logical function phase_ok (phasein)
   ...
   character(len=8) :: phasein, bad_phase(64)
   integer :: nbad, i
   ...
   data bad_phase /'x       ','LZ      ','LN      ','LE      ','LR      ','AML     ','AMB     ','AMS     ',&
                   ...
                   'IAML    ','ml      ','XPR1    ','IAmb_Lg ','Amp     ','IVmBBB  ','IVmB_BB ','IVMs_BB ',&
                   'Pmax    ','Smax    ','Lmax    ','IAMs_20 ','AMl     ','SPECP   ','Amb     ','Vmb_V   ',&
                   'AMP     ','A       ','IVmb_Lg ','Vmb_Lg  ','********','********','********','********'/
   data nbad /60/
