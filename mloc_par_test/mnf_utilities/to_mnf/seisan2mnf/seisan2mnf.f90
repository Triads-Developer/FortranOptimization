...
! Files written by this program are compatible with MNF versions 1.3 through 1.3.3.
   ...
   logical loop, event_on, loop_phase, bulletin, ex, cfil, code_convert, phase_ok
   ...
   write (*,'(a/)') 'Release date March 2, 2020, writing MNF v1.3.3 (backward compatible to v1.3)'
   ...
   do while (loop) ! loop over all lines
      ...
      if (linein(80:80) .eq. '7') then ! Phase lines begin
         ...
         do while (loop_phase)
            ...
            if (phase_ok (phase)) call phase_line (2) ! Write a phase line
         end do
      end if
...
logical function phase_ok (phasein)
   ...
   character(len=8) :: phasein, bad_phase(8)
   integer :: nbad, i
