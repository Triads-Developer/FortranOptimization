...
! 11/16/2017: update to MNF v1.4.2, adding deployment code to phase records
   ...
   if (op) then
      ...
      msg = 'mlocout_gccel: file '//trim(outfil)//' was not opened'
      call oops (trim(msg))
   end if

   ! Attributions
   write (io_gccel,'(2a)') '# Program: ', trim(version)
   ...
   if (ex) then
      ...
      call warnings ('mlocout_gccel: commentary file not found')
   end if
