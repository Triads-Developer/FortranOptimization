...
! 11/17/2017: format change for v10.4.0, adding deployment code
   ...
   logical :: bdp
   integer :: it, iev, n_constrained, n_input, indx(ntmax0), i, ii, j, it1, io_dcal, lunit, n_res
   real :: dts(0:itmax1), dtslim, deltiev(ntmax0), h_mult, deltkm
   real :: x_dep_constrained(nevmax), x_dep_input(nevmax), xmed, cluster_default_depth, lon_out
   real :: sum_res1, mean_res1, s_parent1, z_stat1, sum_res2, mean_res2, s_parent2, z_stat2
   ...
   character(len=100) :: outfil, outfil2, outfil4, oldr_filnam
   character(len=120) :: gfmt, bfmt
   character(len=132) :: msg
   
   data rpres/'PRES'/
   data s_parent1/1.0/ ! Standard deviation of parent population for zM test, based on raw residuals
   data s_parent2/1.0/ ! Standard deviation of parent population for zM test, based on eci (cluster residuals)
...
!    end do
   ...
   do iev = 1, nev
      if (depset_pr(iev) .ne. 'c' .and. depset_pr(iev) .ne. 'u' .and. depset_pr(iev) .ne. 'i' .and. depset_pr(iev) .ne. ' ') then
   ...
   if (direct_cal) then
      ...
      write (io_log,'(a,t26,a,1x,a,1x,a,3x,a,6x,a,1x,a,3x,a)') 'Event', 'N', 'Mean Res', 'Sres', 'zres', 'Mean ECI', 'Seci', 'zeci'
      write (*,'(/a)') 'Possible need for depth adjustment based on zM tests (of raw residuals and ECI values):'
      write (*,'(a,t26,a,1x,a,1x,a,3x,a,6x,a,1x,a,3x,a)') 'Event', 'N', 'Mean Res', 'Sres', 'zres', 'Mean ECI', 'Seci', 'zeci'
   end if
            ...
            if (it .eq. 0) then ! Forward problem
               ...
               if (phase(iev,i) .eq. 'pP      ' .or. phase(iev,i) .eq. 'sP      ' .or. phase(iev,i) .eq. 'pwP     ' .or.&
                  ...
                  write (io_depth_phase,gfmt) bdp_flag, stname(iev,i), deployment(iev,i), fcode(iev,i), no_phid(iev,i),&
                   ...
                   resisc(iev,i), dt(iev,i,it), readsrc(iev,i), channel(iev,i), phase0(iev,i), mnf_line(iev,i),&
                   diff_line(iev,i)
               end if
               ...
               if (phase(iev,i) .eq. 'pP      ' .or. phase(iev,i) .eq. 'sP      ' .or. phase(iev,i) .eq. 'pwP     ' .or.&
                  ...
                  write (io_depth_phase,gfmt) bdp_flag, stname(iev,i), deployment(iev,i), fcode(iev,i), no_phid(iev,i),&
                   phase(iev,i), sdread(iev,i), delt(iev,i), nint(azes(iev,i)), psd(iev,i), weight(iev,i), stacs(iev,i),&
                   resisc(iev,i), (dt(iev,i,j), j=0,it), dtmph(iev,i), dtmpc(iev,i), eci(iev,i), readsrc(iev,i),&
                   channel(iev,i), phase0(iev,i), mnf_line(iev,i), diff_line(iev,i)
               end if
                
               ! Direct calibration readings written to dcal_phase_data file.
               ! Increment sum of residuals.
               if (direct_cal .and. .not.fltrh(iev,i)) then
                  write (io_dcal,gfmt) ' ', stname(iev,i), deployment(iev,i), fcode(iev,i),&
                   no_phid(iev,i), phase(iev,i), sdread(iev,i), delt(iev,i), nint(azes(iev,i)), psd(iev,i), weight(iev,i),&
...
!                  if (.not.fltrhflag(iev,i) .and. .not.fltrcflag(iev,i) .and. delt(iev,i) .gt. windloclim) then ! Not already flagged
            ...
            if (phase(iev,i) .eq. 'pP      ' .or. phase(iev,i) .eq. 'sP      ' .or. phase(iev,i) .eq. 'pwP     ' .or.&
               ...
               write (io_depth_phase,bfmt) bdp_flag, stname(iev,i), deployment(iev,i), fcode(iev,i), no_phid(iev,i),&
                phase(iev,i), sdread(iev,i), delt(iev,i), nint(azes(iev,i)), psd(iev,i), weight(iev,i), stacs(iev,i),&
                resisc(iev,i), (dt(iev,i,j), j=0,it), reason, readsrc(iev,i), channel(iev,i), phase0(iev,i),&
                mnf_line(iev,i), diff_line(iev,i)
            end if
