...
! down to the classical "smallest residual" criterion.
   ...
   if (nphase .eq. 0) then ! Phase ID not changed.
      ...
      if (phasein(1:1) .eq. 'p' .or. phasein(1:1) .eq. 's') then
         ...
         ! P_ is not in the standard phase list so check it separately. Also check if a Pg or Pn might be re-identified as P_         
         if (phasein .eq. 'P_      ' .or. phasein .eq. 'Pg      ' .or. phasein .eq. 'Pn      ') then
            if (delt(iev,ird) .gt. p__min) then
               probi = prob('P_      ', 0, ttobs, delt(iev,ird), depthp(iev,it))
			   if (probi .gt. probmax) then
				  probmax = probi
				  phaseout = 'P_      '
			   end if
         ...
         ! Lg is not in the standard phase list so check it separately. Also check if an Sg or Sn might be re-identified as Lg
         ...
         ! Rg is not in the standard phase list so check it separately. Also check if a Pg might be re-identified as Rg
         if (phasein .eq. 'Rg      ' .or. phasein .eq. 'Pg      ') then
            if (delt(iev,ird) .gt. rg_min) then
			   probi = prob('Rg      ', 0, ttobs, delt(iev,ird), depthp(iev,it))
			   if (probi .gt. probmax) then
				  probmax = probi
				  phaseout = 'Rg      '
			   end if
...
!        h - focal depth
   ...
   character(len=1) :: c
   integer :: mb, itype, i
   real :: b(8), xmed, psprd, t, r, d, robs, x, f, h, ttp_, ttlg, ttrg, tttp
   logical :: ptype, stype
...
!      end if
   ...
   if (verbose_log .and. plogout) then
...
!   Flagging certain phases that are always skipped in mloc
   ...
   integer :: iev, ird, i
   real :: p_arrtime, dp_arrtime, hms2s, delay_time
   logical :: skipp
   ...
   if (verbose_log) then
      if (phase(iev,ird) .ne. phase0(iev,ird)) then
         if (plogout) write (io_plog,'(a,i3,1x,a)') 'pnclean changed '//phase0(iev,ird)//' to '//phase(iev,ird)//' for event ',&
          iev, stname(iev,ird)
      end if
   end if
   ...
   if (phase0(iev,ird) .eq. 'pP      ' .or. phase0(iev,ird) .eq. 'sP      ' .or. phase0(iev,ird) .eq. 'pwP     ') then
      if (stname(iev,ird) .eq. psta) then
         ...
         phidird(iev,ird) = .false. ! Don't re-identify this phase
         if (debug) write (io_log,'(a,1x,a,1x,a,1x,f10.3,2i3,f7.3,1x,l1,1x,l1)') stname(iev,ird), phase0(iev,ird),&
   ...
   if (skipp(phase(iev,ird))) then
      if (verbose_log) write (io_log,'(3a)') 'phase_utility: phase always skipped - ', stname(iev,ird), phase(iev,ird)
      fcode(iev,ird) = 'p'
      phidird(iev,ird) = .false.
   end if
