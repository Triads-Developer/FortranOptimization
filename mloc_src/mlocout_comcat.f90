...
! Similar to a _datf.mnf file, with addition of information about crustal model, empirical reading errors,
...
! 11/16/2017: update to MNF v1.4.2, adding deployment code to phase records
   ...
   ! Attributions
   write (io_comcat,'(2a)') '# Program: ', trim(version)
   write (io_comcat,'(2a)') '# Run: ', trim(basename)
   write (io_comcat,'(2a)') '# Author: ', trim(mloc_author)
   call date_and_time (date, time, zone, values)
   write (io_comcat,'(4a)') '# Date: ', date, ' ', time
   write (io_comcat,'(a,t121,a)') '# ', ' '
   ...
   if (ex) then
      ...
      call warnings ('mlocout_comcat: commentary file not found')
   end if
